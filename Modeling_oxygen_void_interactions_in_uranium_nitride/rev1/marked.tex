...
%DIF DEL old.tex    Fri May  8 18:35:12 2026
%DIF ADD main.tex   Fri May  8 18:37:01 2026
...
\DIFadd{UN adopts the NaCl structure (space group $Fm\bar{3}m$). }\DIFaddend $3 \times 3 \times 3$ supercells of ferromagnetic (FM) UN\DIFaddbegin \DIFadd{, containing 108 formula units (216 atoms), }\DIFaddend are used to model bulk UN and to study the properties of defects and impurities. \DIFaddbegin \DIFadd{The relaxed lattice parameter is $a_0$ = 4.861~\AA\ with a site-projected uranium magnetic moment of $\mu_\text{U} = 1.21 \mu_\text{B}$, reproducing the PBE FM benchmarks of Kocevski \textit{et al.} \cite{Kocevski2022I} and deviating by $-0.6$\% from the experimental value of 4.89 \AA\ \cite{Hayes1990I}.
}

\DIFaddend To study surface properties, 6-layer and 8-layer symmetric slabs \DIFdelbegin \DIFdel{of FM UN }\DIFdelend \DIFaddbegin \DIFadd{based on the same $3 \times 3$ surface unit cell }\DIFaddend are used, \DIFaddbegin \DIFadd{containing 216 and 288 atoms, respectively, }\DIFaddend where the vacuum gap in each is twice the length of the slab along the $z$-direction. It was shown by Kocevski \textit{et al.}
...
Finally, it is of interest to estimate the change in the surface energy upon oxygen adsorption or surface defect formation. This is accomplished by using the following expression, which is derived in \ref{App2} \DIFaddbegin \DIFadd{(using geometric identities derived in \ref{App1})}\DIFaddend :
\begin{equation}
...
\begin{table}[h!]
\centering
\caption{Adsorption energy of oxygen into sites on the symmetric 6-layer slab model of the UN (001) surface \DIFaddbeginFL \DIFaddFL{at a coverage of $\theta = 1/9$~ML (one O atom on one side of the $3 \times 3$ surface unit cell)}\DIFaddendFL . These values correspond to \DIFdelbeginFL \DIFdelFL{$\mu_\text{O} = - 4.18$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\mu_\text{O} = -4.18$~}\DIFaddendFL eV, which is calculated according to the Finnis \textit{et al.}\DIFaddbeginFL \DIFaddFL{~}\DIFaddendFL \cite{Finnis2005} oxide method.}
\footnotesize
\begin{tabular}{lc}
\hline
...
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.5\textwidth]{DO.png}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{(Color online) Diffusivity of oxygen impurities in bulk UN by the defects $ \{\text{O}_\text{N} \! : \! v_\text{N}\} $ and $ \text{O}_i $ under U-rich (blue), Intermediate (orange), and N-rich (green) conditions.}}
    %DIFAUXCMD
%DIFDELCMD < \label{Fig:DO}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The variation in surface energy due to oxygen adsorption and vacancy formation on void surfaces is depicted in \cref{Fig:Delta_sigma} for various porosities, oxygen concentrations, and average void radii. At low temperatures, the change in surface energy is negligible. This is because oxygen diffusivity, incorporated in the model via the kinetic correction, $\alpha_i$, is very slow at low temperatures. Within the void nucleation time, the mobility of oxygen impurities is insufficient for them to reach the void surfaces. For the reference case of $w_\text{O}$ = 1500 ppm, $p$ = 5\%, $R_v$ = 1 nm (\cref{Fig:1500_0.05_1}), the surface energy change is most significant at intermediate temperatures \DIFdelbegin \DIFdel{, around 1500 K, where breakaway swelling in UN is predicted to start \cite{Rizk2025} .
}\DIFdelend \DIFaddbegin \DIFadd{(1300--1700~K), coinciding with the breakaway swelling regime in nitride fuels. Ronchi \textit{et al.}~\cite{Ronchi1978} reported the breakaway swelling critical temperature $T_c$ for mixed nitrides in the interval 1473--1573~K, with an uncertainty of $\pm$50~K. This range, shown as a shaded band in \cref{Fig:Delta_sigma}, overlaps with the temperature of maximum $|\Delta \sigma|$ predicted by our model. The mechanistic modeling by Rizk \textit{et al.}~\cite{Rizk2025} corroborates this picture, attributing the breakaway transition in UN to a change in the gas atom diffusion mechanism occurring around 1500~K. Noting that the range of operating temperatures in liquid metal-cooled fast reactors is 800--2000~K~\cite{Turos1990}, these findings highlight oxygen adsorption as a critical factor in facilitating void nucleation under conditions typical of reactor environments.
}\DIFaddend 

The \DIFdelbegin \DIFdel{oxygen effect on stabilizing voids is more significant at intermediate temperatures due to the balance between oxygen solubility and kinetics . At very }\DIFdelend \DIFaddbegin \DIFadd{temperature dependence of $|\Delta\sigma|$ is governed by the competition between oxygen adsorption kinetics and bulk solubility. At low temperatures, diffusion is too slow for significant oxygen transport to void surfaces; at }\DIFaddend high temperatures, the increased \DIFdelbegin \DIFdel{oxygen solubility in the bulk results in a preference for oxygen to remain in }\DIFdelend \DIFaddbegin \DIFadd{bulk solubility of oxygen disfavors surface segregation. As a result, $|\Delta\sigma|$ exhibits a maximum at an intermediate temperature $T_\text{peak}$.
}

\DIFadd{A closed-form expression for this peak temperature is derived in \ref{App:Tpeak} by analyzing the piecewise monotonicity of $|\Delta\sigma|(T)$. The maximum coincides with the kinetic saturation temperature $T^*$ at which oxygen diffusion first reaches the mean void spacing within the nucleation time, i.e., $\sqrt{D_1(T^*)\,t} = \lambda_v$. Solving this condition yields:
}\begin{equation}
\DIFadd{T_\text{peak} = \frac{E_m + E_f(v_\text{N}^{(\text{b})})}{k\,\ln\!\left[\dfrac{f\,z\,\lambda^2\,\nu\,t}{6\,c_\text{O}\,\lambda_v^2}\right]},
\label{Eq:TpeakMain}
}\end{equation}
\DIFadd{where $f$ is the correlation factor, $z$ the coordination number, $\lambda$ the jump distance, and $\nu$ the attempt frequency for }\DIFaddend the \DIFdelbegin \DIFdel{bulk rather than adsorb onto the void surfaces. The temperature that corresponds to the maximum surface energy reduction is the same for each stoichiometric condition, independent of porosity and oxygen concentration (\cref{Fig:500_0.05_1,Fig:1500_0.05_1,Fig:3000_5_1}).
This stems from the fact that the kinetic factors, $\alpha_i$, have no dependence on either porosity or oxygen concentration. The kinetic factors, however, depend on the average pore radius, i.e., $\alpha_i \sim 1/\lambda_v \sim 1/R_v$. As shown in \cref{Fig:1500_0.05_1,Fig:1500_0.05_10}}\DIFdelend \DIFaddbegin \DIFadd{$\{\text{O}_\text{N} \! : \! v_\text{N}\}$ mechanism (defined in \cref{Sec:Diff}), and $\lambda_v = n_v^{-1/3}$ is the mean free path between voids. The full derivation is given in \ref{App:Tpeak}. Because $R_v$}\DIFaddend , \DIFaddbegin \DIFadd{$p$, $c_\text{O}$, and $t$ enter only through the logarithm, $T_\text{peak}$ is controlled primarily by the activation energy scale $E_m + E_f(v_\text{N}^{(\text{b})})$ and is only weakly sensitive to microstructural parameters and oxygen concentration.
}

\DIFadd{The weak dependence of $|\Delta\sigma|$ on porosity is also explained analytically in \ref{App:Tpeak:Porosity}. The leading $(1-p)/p$ factor in the geometric prefactor of \cref{Eq:DeltaSigma1} is exactly canceled by an opposing $p/(1-p)$ factor arising from the surface nitrogen site fraction (\cref{Eq:NNs1}) that enters the segregation ratio (\cref{Eq:ONs_ONb}). The residual porosity dependence is only $1/[\text{N}_\text{N}^{(\text{b})}] \approx 1$, since $[\text{N}_\text{N}^{(\text{s})}] \ll 1$ for the void sizes of interest. Furthermore, at $T_\text{peak}$, the kinetic factor is saturated ($\alpha_1 = 1$), so the $p^{1/3}$ dependence of $\alpha_1$ does not contribute at the peak. Numerical evaluation confirms that the peak $|\Delta\sigma|$ varies by less than 10\% across $p = 5$--20\% for $R_v = 25$~nm (\cref{Tab:App:por}).
}

\DIFadd{The dependence on void size is more significant. Since $\lambda_v \sim R_v / p^{1/3}$, larger voids increase the denominator of \cref{Eq:TpeakMain}, shifting $T_\text{peak}$ to higher temperatures. This also reduces the peak magnitude of $|\Delta\sigma|$ because the segregation drive is weaker at higher temperatures. For example, }\DIFaddend increasing $R_v$ from 1\DIFaddbegin \DIFadd{~}\DIFaddend nm to 10\DIFdelbegin \DIFdel{nm shifts the temperature of maximum surface energy reduction by about 200 K to the right, and slightly decreases the overall magnitude of $\Delta \sigma$. This indicates that larger voids require higher temperatures for oxygen adsorption to meaningfully lower their surface energy. Nevertheless, even at elevated temperatures, the reduction in surface energy for larger voids remains smaller than that for smaller voids at the same porosity and oxygen concentration. Thus, oxygen-induced surface stabilization is }\DIFdelend \DIFaddbegin \DIFadd{~nm shifts $T_\text{peak}$ upward by approximately 180~K (\cref{Fig:1500_0.05_1,Fig:1500_0.05_10}). Note this 180~K upward shift observed from the figures can be reproduced quantitatively using \cref{Eq:TpeakMain}, which validates \cref{Eq:TpeakMain}. Oxygen-induced surface stabilization is therefore }\DIFaddend most effective for small \DIFdelbegin \DIFdel{cavities, i.e., }\DIFdelend \DIFaddbegin \DIFadd{voids }\DIFaddend during the early stages of \DIFdelbegin \DIFdel{void nucleation and bubble }\DIFdelend \DIFaddbegin \DIFadd{nucleation and }\DIFaddend growth.
...
\centering
\begin{subfigure}{0.48\textwidth}
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Delta_sigma_5_500_1e-09.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Delta_sigma_0.05_500.0_1e-09.png}
    \DIFaddendFL \caption{$w_\text{O}$ = 500 ppm, $p$ = 5\%, $R_v$ = 1 nm}
    \label{Fig:500_0.05_1}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Delta_sigma_5_1500_1e-09.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Delta_sigma_0.05_1500.0_1e-09.png}
    \DIFaddendFL \caption{$w_\text{O}$ = 1500 ppm, $p$ = 5\%, $R_v$ = 1 nm}
    \label{Fig:1500_0.05_1}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Delta_sigma_5_3000_1e-09.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Delta_sigma_0.05_3000.0_1e-09.png}
    \DIFaddendFL \caption{$w_\text{O}$ = 3000 ppm, $p$ = 5\%, $R_v$ = 1 nm}
    \label{Fig:3000_5_1}
\end{subfigure}
\begin{subfigure}{0.48\textwidth}
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Delta_sigma_5_1500_1e-08.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Delta_sigma_0.05_1500.0_1e-08.png}
    \DIFaddendFL \caption{$w_\text{O}$ = 1500 ppm, $p$ = 5\%, $R_v$ = 10 nm}
...
\DIFaddend For the reference case of $w_\text{O}$ = 1500 ppm, $p$ = 5\%, and $R_v$ = 1 nm, $|\Delta \sigma|$ is found to be around 0.4~J/m$^2$, occurring within the temperature interval 1300--1700~K (see \cref{Fig:1500_0.05_1}). To assess the sensitivity of $|\Delta \sigma|$ to key parameters, we vary $w_\text{O}$ and $p$ while holding $R_v$ fixed at 25 nm, which is a typical size for both dislocations and intergranular bubbles \cite{Colin1983}. It has been shown that dislocation bubbles dominate bubble swelling in UN \cite{Ronchi1975,Ronchi1978,Rizk2025}. Although the assumptions of our methodology are more representative of bulk voids, we assume that the same surface energy reduction mechanism applies to intragranular (bulk and dislocation) and intergranular bubbles.

\Cref{Fig:p_w_plot} represents the maximum $|\Delta \sigma|$, which occurs at intermediate stoichiometric conditions. As explained earlier, because $R_v$ is kept fixed, the temperature at which this maximum occurs is \DIFdelbegin \DIFdel{the same (1800 K in the case of }\DIFdelend \DIFaddbegin \DIFadd{approximately the same for all $(w_\text{O}, p)$ combinations shown, varying only between $\sim$1620 and $\sim$1750~K for }\DIFaddend $R_v$ = 25\DIFdelbegin \DIFdel{nm), independent of }\DIFdelend \DIFaddbegin \DIFadd{~nm at intermediate stoichiometry. This weak dependence on }\DIFaddend $w_\text{O}$ \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $p$ \DIFaddbegin \DIFadd{follows from \cref{Eq:TpeakMain}, where both parameters enter only through a logarithm}\DIFaddend . The color map reveals that $|\Delta \sigma|$ is \DIFdelbegin \DIFdel{nearly independent of porosity }\DIFdelend \DIFaddbegin \DIFadd{weakly dependent on porosity for $R_v$ = 25 nm}\DIFaddend . The vertical red line marks the oxygen concentration in UN samples from Ronchi \textit{et al.} ($w_\text{O} = 1800$ ppm) \cite{Ronchi1975,Ronchi1978}, used by Rizk \textit{et al.} \cite{Rizk2025} to validate their swelling model. Fission gas bubbles with an average radius of 25~nm are predicted to experience a reduction in surface energy of about 0.3--0.4~J/m$^2$, approximately 20\% of our calculated surface energy in the oxygen-free case.
...
\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.5\textwidth]{p_w_abs_gradual_plot_2_5e-08.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.5\textwidth]{p_w_abs_gradual_plot_2.5e-08.png}
    \DIFaddendFL \caption{(Color online) Maximum surface energy reduction, $|\Delta \sigma|$ as a function of porosity and oxygen concentration under intermediate stoichiometry for a fixed bubble radius $R_v$ = 25 nm, typical of dislocation and grain-boundary bubbles. The red line marks $w_\text{O}$ = 1800 ppm, the oxygen concentration in UN samples from Ronchi \textit{et al.} \cite{Ronchi1975,Ronchi1978}, used by Rizk \textit{et al.} \cite{Rizk2025} to validate their swelling model.\DIFdelbeginFL \DIFdelFL{Because $R_v$ is constant, all maxima in $|\Delta \sigma|$ occur at the same temperature.}\DIFdelendFL }
    \label{Fig:p_w_plot}
\end{figure}
...
\section{Discussion}
\DIFaddbegin \label{Sec:Discuss}
\DIFaddend 

This study presents a novel model that integrates first-principles calculations with defect chemistry to quantify the role of oxygen impurities in stabilizing voids and fission gas bubbles in UN. To our knowledge, this is the first model to establish a quantitative link between oxygen concentration and void nucleation thermodynamics in UN. By capturing how oxygen segregates to void surfaces and reduces the surface energy, this work advances the mechanistic understanding of swelling phenomena in oxygen-contaminated nuclear fuels. \DIFdelbegin \DIFdel{Importantly, this surface energy reduction is found to be most pronounced for void embryos ($R_v$ = 1 nm) at intermediate temperatures (1300--1700~K), aligning with the temperature range associated with the onset of breakaway swelling in UN \cite{Ronchi1975,Ronchi1978,Rizk2025}. Noting that the range of operating temperatures in liquid metal-cooled fast reactors is 800--2000 K \cite{Turos1990}, these findings highlight oxygen adsorption as a critical factor in facilitating void nucleation under conditions typical of reactor environments}\DIFdelend \DIFaddbegin \DIFadd{As shown in \cref{Sec:Oxygen}, the temperature of maximum $|\Delta \sigma|$ coincides with the experimentally reported breakaway swelling critical temperature for nitride fuels~\cite{Ronchi1978} and with the diffusion mechanism transition identified by Rizk \textit{et al.}~\cite{Rizk2025}, reinforcing the connection between oxygen-induced surface stabilization and the onset of accelerated swelling}\DIFaddend .
...
\DIFaddend The model provides predictive capabilities by mapping the dependence of surface energy reduction on key physical parameters, including temperature, porosity, void radius, and oxygen concentration. Parametric analysis reveals that surface energy reduction is strongly influenced by oxygen concentration and void radius, while being \DIFdelbegin \DIFdel{nearly independent of }\DIFdelend \DIFaddbegin \DIFadd{weakly dependent on }\DIFaddend porosity. In particular, small bulk voids and bubbles experience a significant stabilization effect from oxygen at intermediate temperatures, highlighting the role of oxygen in the early stages of void or fission gas bubble formation. These insights may provide a physically grounded explanation for the association between oxygen content and accelerated swelling observed in experimental studies of mixed uranium carbide/nitride fuels~\cite{Rogozkin2003}. Furthermore, the generality of the approach allows for its extension to other materials systems and integration into mesoscale models and fuel performance codes, offering a path toward multiscale modeling of chemically assisted swelling in nuclear materials.
...
\DIFaddbegin \DIFadd{To quantify the impact of oxygen-induced surface energy reduction on void nucleation, we invoke classical nucleation theory. The free energy barrier for homogeneous void nucleation is \cite{Fultz2020}:
}\begin{equation}
\DIFadd{\Delta G^* = \frac{16 \pi \sigma^3}{3 (\Delta g_v)^2},
}\end{equation}
\DIFadd{where $\Delta g_v$ is the volumetric free energy driving force for void formation. Since $\Delta G^* \sim \sigma^3$, the ratio of nucleation barriers with and without oxygen is:
}\begin{equation}
\DIFadd{\frac{\Delta G^*_\text{O}}{\Delta G^*_\text{clean}} = \left( \frac{\sigma_\text{clean} + \Delta \sigma}{\sigma_\text{clean}} \right)^3 = \left( 1 + \frac{\Delta \sigma}{\sigma_\text{clean}} \right)^3.
}\end{equation}
\DIFadd{For $\sigma_\text{clean} = 1.59$~J/m$^2$ and $\Delta \sigma \approx -0.4$~J/m$^2$ (intermediate stoichiometry, $w_\text{O} = 1500$~ppm, $R_v = 1$~nm), this gives $\sigma_\text{O} \approx 1.19$~J/m$^2$ and:
}\begin{equation}
\DIFadd{\frac{\Delta G^*_\text{O}}{\Delta G^*_\text{clean}} = \left( \frac{1.19}{1.59} \right)^3 \approx 0.42,
}\end{equation}
\DIFadd{corresponding to a $\sim$58\% reduction in the nucleation barrier. This substantial lowering of $\Delta G^*$ translates into a dramatically increased nucleation rate, $J \sim \exp(-\Delta G^*/kT)$ \cite{Fultz2020}, consistent with the experimentally observed enhancement of void swelling in oxygen-contaminated UN.
}

\DIFaddend % An elevated surface energy would have significant consequences for the thermodynamic and kinetic evolution of fission gas bubbles in the mechanistic model. The equilibrium internal pressure of a bubble, given by the modified Young–Laplace relation $P_{\text{eq}} = 2\sigma / R_v - s_h$, scales linearly with $\sigma$. Thus, increasing the surface energy raises the equilibrium pressure required for mechanical stability at a given radius, resulting in more severe over-pressurization for the same gas content. This, in turn, enhances the driving force for vacancy absorption while simultaneously increasing the energetic penalty associated with surface formation. The net effect is a suppression of bubble nucleation and growth, particularly for dislocation-mediated intragranular bubbles that dominate swelling near the breakaway temperature. Moreover, grain boundary bubbles, which are already over-pressurized in the model, would experience even greater resistance to expansion, thereby delaying interconnection and fission gas release. These effects would collectively shift the Vitanza-type release threshold to higher temperatures and burnups, leading to unrealistically low gas release predictions and artificial retention of gas within the fuel matrix. 
...
\begin{itemize}
    ...
    \item \textit{Porosity ($p$):} The surface energy reduction is found to be \DIFdelbegin \DIFdel{largely insensitive to }\DIFdelend \DIFaddbegin \DIFadd{weakly dependent on }\DIFaddend porosity.
    \end{itemize}
...
\DIFaddend The area associated with an N atom on the surface is $a^2/2$, where $a$ is the lattice constant. The total surface area of the internal voids is $N_v \times 4 \pi R_v^2$, where $N_v$ is the total number of voids, calculated as:
\begin{equation}
...
\DIFaddbegin \section{\DIFadd{Closed-form expression for the temperature of maximum $|\Delta \sigma|$}}
\label{App:Tpeak}

\DIFadd{This appendix derives a closed-form expression for the temperature at which $|\Delta \sigma|$ is maximized. The derivation proceeds in four steps: we first state the assumptions, then factor the reduced $|\Delta \sigma|(T)$ into terms with known monotonicity, establish piecewise monotonicity on either side of a kinetic saturation temperature~$T^*$, and conclude that $T_\text{peak} = T^*$. The closed-form result for $T^*$ is given in \cref{Eq:App:TstarFormula}, and its implications for the weak porosity dependence of $|\Delta \sigma|$ are discussed in \ref{App:Tpeak:Porosity}.
}

\subsection{\DIFadd{Assumptions}}
\label{App:Tpeak:Assumptions}

\DIFadd{The derivation rests on the following assumptions, each consistent with the modeling choices in \cref{Sec:Oxygen}:
}\begin{enumerate}[label=(\roman*)]
    \item \DIFadd{\textit{Dominant channel.} Only the $\text{O}_\text{N}^{(\text{s})}$ term in \cref{Eq:DeltaSigma1} is retained; the $\text{O}_i^{(\text{s})}$ and $v_\text{N}^{(\text{s})}$ contributions are neglected. This is justified by \cref{Fig:D}, where $\Delta \sigma(\text{O}_\text{N}^{(\text{s})})$ dominates by at least an order of magnitude.
    }\item \DIFadd{\textit{Fixed stoichiometry.} The vacancy formation energies $E_f(v_\text{N}^{(\text{b})})$ and $E_f(v_\text{N}^{(\text{s})})$ are held at their values for a given stoichiometric condition.
    }\item \DIFadd{\textit{Dilute defects.} Concentrations follow Arrhenius statistics with unit prefactor, e.g., $[v_\text{N}^{(\text{b})}] = \exp[-E_f(v_\text{N}^{(\text{b})})/(kT)]$ and $[\text{O}_\text{N}^{(\text{b})}] \approx c_\text{O}$.
    }\item \DIFadd{\textit{Single transport channel.} The kinetic correction $\alpha_1$ is computed from the $\{\text{O}_\text{N} : v_\text{N}\}$ diffusivity (\cref{Sec:Oxygen}).
    }\item \DIFadd{\textit{Existence of $T^*$.} The argument of the logarithm in \cref{Eq:App:TstarFormula} must exceed unity, which is verified for all parameter combinations considered in this work.
}\end{enumerate}

\subsection{\DIFadd{Factorized form}}
\label{App:Tpeak:Factorization}

\DIFadd{Retaining only the dominant $\text{O}_\text{N}^{(\text{s})}$ term, \cref{Eq:DeltaSigma1} takes the form:
}\begin{equation}
\DIFadd{|\Delta \sigma|(T) = \underbrace{\frac{8}{3}\frac{1-p}{p}\frac{R_v}{a^3}\,c_\text{O}}_{\displaystyle\mathcal{P}}\;\alpha_1(T)\;g(T)\;|E_\text{ad}(T)|,
\label{Eq:App:Factorization}
}\end{equation}
\DIFadd{where $\mathcal{P}$ is a $T$-independent prefactor. Three factors carry temperature dependence: the kinetic correction $\alpha_1$, the segregation ratio $g \equiv [\text{O}_\text{N}^{(\text{s})}]/[\text{O}_\text{N}^{(\text{b})}]$, and the adsorption energy $E_\text{ad}$ evaluated with respect to the impurity chemical potential (\cref{Sec:uO}). We now derive the explicit $T$-dependence of each.
}

\paragraph{\DIFadd{Kinetic factor}} \DIFadd{The kinetic correction $\alpha_1(T) = \min[\sqrt{D_1(T)\,t}/\lambda_v,\; 1]$ (\cref{Eq:alpha}) is continuous but non-differentiable at the \emph{saturation temperature} $T^*$ defined by $\sqrt{D_1(T^*)\,t} = \lambda_v$. The effective oxygen diffusivity via the $\{\text{O}_\text{N} : v_\text{N}\}$ mechanism is:
}\begin{equation}
\DIFadd{D_1(T) = f\,\frac{[v_\text{N}^{(\text{b})}]}{c_\text{O}}\,D_{\{\text{O}_\text{N}:v_\text{N}\}} = \frac{f\,z\,\lambda^2\,\nu}{6\,c_\text{O}}\,\exp\!\left[-\frac{E_m + E_f(v_\text{N}^{(\text{b})})}{kT}\right],
\label{Eq:App:D1}
}\end{equation}
\DIFadd{where the second equality uses $D_d = \frac{1}{6}z\lambda^2\nu\exp[-E_m/(kT)]$ and $[v_\text{N}^{(\text{b})}] = \exp[-E_f(v_\text{N}^{(\text{b})})/(kT)]$. In the unsaturated regime ($T < T^*$):
}\begin{equation}
\DIFadd{\alpha_1(T) = C_\alpha\,\exp\!\left[-\frac{E_m + E_f(v_\text{N}^{(\text{b})})}{2kT}\right], \quad C_\alpha \equiv \frac{1}{\lambda_v}\sqrt{\frac{f\,z\,\lambda^2\,\nu\,t}{6\,c_\text{O}}}.
\label{Eq:App:alpha1Unsat}
}\end{equation}

\paragraph{\DIFadd{Segregation factor}} \DIFadd{Combining \cref{Eq:ONs_ONb} with the $K_5$ equilibrium (\cref{Eq:VN-VNs}):
}

\begin{align}
\DIFadd{g(T) = }& \DIFadd{\frac{[\text{N}_\text{N}^{(\text{s})}]}{[\text{N}_\text{N}^{(\text{b})}]}\,\exp\!\left[+\frac{E_\text{seg}}{kT}\right], }\\
\DIFadd{E_\text{seg} \equiv }& \DIFadd{-\bigl[E_f(v_\text{N}^{(\text{s})}) - E_f(v_\text{N}^{(\text{b})}) + E_\text{ad}(\text{O}_\text{N}^{(\text{s})}) - E_\text{inc}(\text{O}_\text{N}^{(\text{b})})\bigr],
\label{Eq:App:gT}
}\end{align}

\DIFadd{where both $E_\text{ad}$ and $E_\text{inc}$ in the definition of $E_\text{seg}$ contain $\mu_\text{O}$, which cancels in their difference. That is, $E_\text{seg}$ is independent of the choice of oxygen reference state and is constructed entirely from raw DFT total-energy differences. Note that the prefactor $[\text{N}_\text{N}^{(\text{s})}]/[\text{N}_\text{N}^{(\text{b})}]$ is $T$-independent.
}

%DIF >  The segregation factor $g(T)$ therefore carries no chemical-potential dependence, and the explicit $T$-dependence associated with the impurity reference enters solely through $E_\text{ad}(T)$, as derived below. This separation avoids double-counting of chemical-potential contributions.

\DIFadd{Substituting the DFT values from \cref{Tab:EfVN,Tab:Einc,Tab:Ead}:
}\begin{equation}
\DIFadd{E_\text{seg} = -\bigl[(-0.76) + (0.43)\bigr] = 0.33~\text{eV},
\label{Eq:App:EsegValue}
}\end{equation}
\DIFadd{where the segregation energy of $v_\text{N}$ is $E_f(v_\text{N}^{(\text{s})}) - E_f(v_\text{N}^{(\text{b})}) = -0.76$~eV (\cref{Tab:EfVN}) and $E_\text{ad}(\text{O}_\text{N}^{(\text{s})}) - E_\text{inc}(\text{O}_\text{N}^{(\text{b})}) = -6.80 - (-7.23) = 0.43$~eV (\cref{Tab:Ead,Tab:Einc}). Both differences are stoichiometry-independent, so $E_\text{seg} = 0.33$~eV for all conditions.
}

\paragraph{\DIFadd{Adsorption-energy factor}} \DIFadd{From the definition of the adsorption energy (\cref{Eq:Ead}), the adsorption energy referred to the impurity chemical potential is:
}\begin{equation}
\DIFadd{E_\text{ad}(T) = \frac{1}{N_\text{ad}}\bigl(E_\text{surf+ad} - E_\text{surf}\bigr) - \mu_\text{O}(T),
}\end{equation}
\DIFadd{where $\frac{1}{N_\text{ad}}(E_\text{surf+ad} - E_\text{surf})$ is the raw (reference-free) adsorption energy, a $T$-independent quantity obtained directly from DFT slab calculations. From \cref{Eq:OxygenTruePot}, using $[\text{O}_\text{N}^{(\text{b})}] \approx c_\text{O}$ and $[v_\text{N}^{(\text{b})}] = \exp[-E_f(v_\text{N}^{(\text{b})})/(kT)]$:
}\begin{equation}
\DIFadd{\mu_\text{O}(T) = E(\text{O}_\text{N}) - E(v_\text{N}) + E_f(v_\text{N}^{(\text{b})}) + kT\ln c_\text{O}.
\label{Eq:App:muOimp}
}\end{equation}
\DIFadd{Substituting:
}\begin{equation}
\DIFadd{E_\text{ad}(T) = \underbrace{\frac{1}{N_\text{ad}}\bigl(E_\text{surf+ad} - E_\text{surf}\bigr) - E(\text{O}_\text{N}) + E(v_\text{N}) - E_f(v_\text{N}^{(\text{b})})}_{\displaystyle\equiv\,E_\text{ad}^{(0)},\ T\text{-independent}} - kT \ln c_\text{O}.
\label{Eq:App:EadImpExplicit}
}\end{equation}
\DIFadd{Note that $E_\text{ad}^{(0)}$ is built entirely from raw DFT total energies and the vacancy formation energy; no oxygen chemical potential appears. Since $E_\text{ad} < 0$ (favorable adsorption) and $\ln c_\text{O} < 0$ (dilute impurity), the magnitude $|E_\text{ad}(T)| = -E_\text{ad}^{(0)} + kT\ln c_\text{O}$ is strictly \emph{decreasing} in $T$ (also see \cref{Fig:EduO}), with $d|E_\text{ad}|/dT = k\ln c_\text{O} < 0$.
}

\subsection{\DIFadd{Piecewise monotonicity and peak location}}
\label{App:Tpeak:Monotonicity}

\DIFadd{The function $|\Delta \sigma|(T)$ is continuous but not differentiable at $T = T^*$. We establish its monotonicity separately on each side.
}

\subsubsection{\DIFadd{Unsaturated regime (}\texorpdfstring{$T < T^*$}{T < T*}\DIFadd{)}}

\DIFadd{Substituting \cref{Eq:App:alpha1Unsat,Eq:App:gT,Eq:App:EadImpExplicit} into \cref{Eq:App:Factorization} and taking the logarithmic derivative:
}\begin{equation}
\DIFadd{\frac{d}{dT}\ln|\Delta \sigma| = \underbrace{\frac{1}{kT^2}\!\left[\frac{E_m + E_f(v_\text{N}^{(\text{b})})}{2} - E_\text{seg}\right]}_{\displaystyle > 0 \text{ if \cref{Eq:App:Inequality} holds}} + \underbrace{\frac{k\ln c_\text{O}}{|E_\text{ad}(T)|}}_{\displaystyle < 0}.
\label{Eq:App:LogDerivUnsat}
}\end{equation}
\DIFadd{The first term is positive provided:
}\begin{equation}
\DIFadd{E_m + E_f(v_\text{N}^{(\text{b})}) > 2\,E_\text{seg}.
\label{Eq:App:Inequality}
}\end{equation}
\DIFadd{The second term is negative since $\ln c_\text{O} < 0$. The sign of the sum determines whether $|\Delta \sigma|$ is increasing or decreasing. The inequality \cref{Eq:App:Inequality} is verified in \ref{App:Tpeak:Verification}, where it is shown to be satisfied with a margin of $\sim$2--2.5~eV for all stoichiometric conditions. Furthermore, a numerical evaluation over the full physically relevant parameter domain ($T \in [800, 2500]$~K, $w_\text{O} \in [500, 3000]$~ppm, all three stoichiometries) confirms that the positive first term dominates the negative second term, so the sum in \cref{Eq:App:LogDerivUnsat} is strictly positive throughout the unsaturated regime.
}

\subsubsection{\DIFadd{Saturated regime (}\texorpdfstring{$T > T^*$}{T > T*}\DIFadd{)}}

\DIFadd{In this regime $\alpha_1 = 1$, so:
}\begin{equation}
\DIFadd{\frac{d}{dT}\ln|\Delta \sigma| = -\frac{E_\text{seg}}{kT^2} + \frac{k\ln c_\text{O}}{|E_\text{ad}(T)|}.
\label{Eq:App:LogDerivSat}
}\end{equation}
\DIFadd{Both terms are negative ($E_\text{seg} > 0$ and $\ln c_\text{O} < 0$), so $|\Delta \sigma|$ is \emph{strictly decreasing} for $T > T^*$, unconditionally. This conclusion is independent of the kinetic model.
}

\subsubsection{\DIFadd{Peak location}}

\DIFadd{Since $|\Delta \sigma|$ is strictly increasing for $T < T^*$ and strictly decreasing for $T > T^*$, continuity implies:
}\begin{equation}
\DIFadd{T_\text{peak} = T^*.
\label{Eq:App:TpeakIsTstar}
}\end{equation}
\DIFadd{Because $T^*$ is a kink (non-differentiable point) of $|\Delta \sigma|(T)$, this result is obtained via piecewise monotonicity rather than by setting a derivative to zero.
}

\subsection{\DIFadd{Verification of the monotonicity condition}}
\label{App:Tpeak:Verification}

\DIFadd{The inequality \cref{Eq:App:Inequality} is evaluated for each stoichiometric condition using the DFT values in \cref{Tab:EfVN} and the migration energy $E_m = 3.06$~eV:
}\begin{table}
\footnotesize
\centering
\begin{tabular}{lccc}
...
\hline
\DIFaddFL{$E_m + E_f(v_\text{N}^{(\text{b})})$ }[\DIFaddFL{eV}] & \DIFaddFL{4.70 }& \DIFaddFL{5.19 }& \DIFaddFL{5.68 }\\
\DIFaddFL{$2E_\text{seg}$ }[\DIFaddFL{eV}] & \DIFaddFL{0.66 }& \DIFaddFL{0.66 }& \DIFaddFL{0.66 }\\
\DIFaddFL{$\tfrac{1}{2}[E_m + E_f(v_\text{N}^{(\text{b})})] - E_\text{seg}$ }[\DIFaddFL{eV}] & \DIFaddFL{2.02 }& \DIFaddFL{2.27 }& \DIFaddFL{2.51 }\\
\hline
\end{tabular}
\end{table}
\DIFadd{The margin of $\sim$2--2.5~eV in the first term of \cref{Eq:App:LogDerivUnsat} ensures strong separation from the competing negative term. The negative contribution is bounded in magnitude by $k|\ln c_\text{O}|/|E_\text{ad}|$. For $c_\text{O} \sim 10^{-3}$--$10^{-2}$ and $|E_\text{ad}| \gtrsim 1$~eV, this term is $\lesssim 6 \times 10^{-4}$~K$^{-1}$. In contrast, the first term remains $\gtrsim 10^{-3}$~K$^{-1}$ over $T$ = 800--2500~K, and thus dominates throughout the range of interest.
}

\subsection{\DIFadd{Closed-form expression for }\texorpdfstring{$T^*$}{T*}}
\label{App:Tpeak:Formula}

\DIFadd{Solving $\sqrt{D_1(T^*)\,t} = \lambda_v$ using \cref{Eq:App:D1}:
}\begin{equation}
\DIFadd{T_\text{peak} = T^* = \frac{E_m + E_f(v_\text{N}^{(\text{b})})}{k\,\ln\!\left[\dfrac{f\,z\,\lambda^2\,\nu\,t}{6\,c_\text{O}\,\lambda_v^2}\right]}.
\label{Eq:App:TstarFormula}
}\end{equation}
\DIFadd{Substituting $\lambda_v = (4\pi R_v^3 / 3p)^{1/3}$:
}\begin{equation}
\DIFadd{T_\text{peak} = \frac{E_m + E_f(v_\text{N}^{(\text{b})})}{k\,\ln\!\left[\dfrac{f\,z\,\lambda^2\,\nu\,t}{6\,c_\text{O}}\left(\dfrac{3p}{4\pi R_v^3}\right)^{2/3}\right]}.
\label{Eq:App:TstarExplicit}
}\end{equation}
\DIFadd{The formula is meaningful only when the argument of the logarithm exceeds unity, i.e., when $f\,z\,\lambda^2\,\nu\,t > 6\,c_\text{O}\,\lambda_v^2$. Physically, this means oxygen must be capable of diffusing at least one void spacing within the nucleation time; otherwise $\alpha_1 < 1$ throughout the accessible temperature range and no kinetic saturation occurs. For the parameter ranges in this work ($R_v \in \{1, 10, 25\}$~nm, $p \in [0.05, 0.20]$, $w_\text{O} \in [500, 3000]$~ppm), this condition is satisfied at all relevant temperatures.
}

\subsection{\DIFadd{Dependence of }\texorpdfstring{$T_\text{peak}$}{Tpeak} \DIFadd{on the nucleation time }\texorpdfstring{$t$}{t}}
\label{App:Tpeak:tdep}

\DIFadd{\cref{Eq:App:TstarExplicit} makes explicit that $t$ enters the expression for $T_\text{peak}$ only through a logarithm. Differentiating with respect to $\ln t$:
}\begin{equation}
\DIFadd{\frac{dT_\text{peak}}{d\ln t}
=
...
\DIFadd{where $A_0 \equiv f z \lambda^2 \nu / (6 c_\text{O})$. Thus increasing $t$ lowers $T_\text{peak}$, but only weakly. For an order-of-magnitude change in $t$:
}\begin{equation}
\DIFadd{\Delta T_\text{peak}
\approx
-\frac{k\,T_\text{peak}^2}{E_m + E_f(v_\text{N}^{(\text{b})})}\,\ln 10.
\label{Eq:App:deltaTpeak}
}\end{equation}

\DIFadd{Taking $E_m + E_f(v_\text{N}^{(\text{b})}) = 5.19$~eV (intermediate stoichiometry) and $T_\text{peak} = 1448$~K (the expected value for $R_v$ = 1 nm, $w_\text{O}$ = 1500 ppm, $p$ = 0.05) gives:
}\begin{equation}
\DIFadd{\Delta T_\text{peak} \approx -80~\text{K}.
}\end{equation}
\DIFadd{This confirms that $T_\text{peak}$ is only logarithmically sensitive to the nucleation time.
}

\subsection{\DIFadd{Numerical validation and parametric trends}}
\label{App:Tpeak:Validation}

\DIFadd{\Cref{Eq:App:TstarFormula} was compared against direct numerical maximization of the full $|\Delta \sigma|(T)$ from \cref{Eq:DeltaSigma1} (including the $\text{O}_i^{(\text{s})}$ and $v_\text{N}^{(\text{s})}$ contributions) for a grid of parameter combinations at intermediate stoichiometry:
}

\begin{table}[h!]
\centering
\footnotesize
\caption{\DIFaddFL{Comparison of $T_\text{peak}$ from the closed-form expression (\cref{Eq:App:TstarFormula}) and from direct numerical maximization of the full $|\Delta \sigma|(T)$ (\cref{Eq:DeltaSigma1}), at intermediate stoichiometry.}}
\label{Tab:App:Tpeak}
\begin{tabular}{lccc}
\hline
 & \DIFaddFL{$T_\text{peak}$ (formula) }[\DIFaddFL{K}] & \DIFaddFL{$T_\text{peak}$ (numerical) }[\DIFaddFL{K}] & \DIFaddFL{Difference }[\DIFaddFL{K}] \\
\hline
\DIFaddFL{$R_v = 1$ nm,  $w_\text{O} = 1500$ ppm, $p = 0.05$ }& \DIFaddFL{1447 }& \DIFaddFL{1448 }& \DIFaddFL{$<1$ }\\
\DIFaddFL{$R_v = 1$ nm,  $w_\text{O} = 1500$ ppm, $p = 0.20$ }& \DIFaddFL{1416 }& \DIFaddFL{1416 }& \DIFaddFL{$<1$ }\\
\DIFaddFL{$R_v = 10$ nm, $w_\text{O} = 1500$ ppm, $p = 0.05$ }& \DIFaddFL{1627 }& \DIFaddFL{1627 }& \DIFaddFL{$<1$ }\\
\DIFaddFL{$R_v = 25$ nm, $w_\text{O} = 1800$ ppm, $p = 0.05$ }& \DIFaddFL{1721 }& \DIFaddFL{1721 }& \DIFaddFL{$<1$ }\\
\DIFaddFL{$R_v = 25$ nm, $w_\text{O} = 1800$ ppm, $p = 0.20$ }& \DIFaddFL{1676 }& \DIFaddFL{1677 }& \DIFaddFL{$<1$ }\\
\DIFaddFL{$R_v = 25$ nm, $w_\text{O} = 1500$ ppm, $p = 0.05$ }& \DIFaddFL{1712 }& \DIFaddFL{1712 }& \DIFaddFL{$<1$ }\\
\hline
\end{tabular}
\end{table}

\DIFadd{Agreement is within 1~K in all cases, confirming that the neglected $\text{O}_i^{(\text{s})}$ and $v_\text{N}^{(\text{s})}$ contributions shift the peak negligibly for the DFT parameter set used here.
}

\DIFadd{\Cref{Eq:App:TstarExplicit} yields the following parametric trends, consistent with \cref{Fig:Delta_sigma}:
}\begin{itemize}
    \item \DIFadd{\textit{Increasing $R_v$} increases $\lambda_v$, decreasing the logarithm and therefore increasing $T_\text{peak}$. The $\sim$180~K upward shift from $R_v = 1$~nm to $R_v = 10$~nm reported in \cref{Sec:Oxygen} is reproduced quantitatively.
    }\item \DIFadd{\textit{Increasing $p$} decreases $\lambda_v$ and slightly decreases $T_\text{peak}$. The effect is weak because $p$ enters through a logarithm.
    }\item \DIFadd{\textit{Increasing $c_\mathrm{O}$} increases the denominator of the logarithm's argument and slightly increases $T_\text{peak}$; again, the effect is logarithmic and weak.
    }\item \DIFadd{\textit{Stoichiometry} enters the numerator through $E_f(v_\text{N}^{(\text{b})})$: N-rich conditions (higher $E_f$) shift $T_\text{peak}$ upward.
}\end{itemize}

\subsection{\DIFadd{Consequence for the porosity dependence of }\texorpdfstring{$|\Delta \sigma|$}{|Delta sigma|}}
\label{App:Tpeak:Porosity}

\DIFadd{The prefactor of \cref{Eq:DeltaSigma1} contains the factor $(1-p)/p$, which varies by nearly $4.75\times$ between $p = 5\%$ and $p = 20\%$. Yet the peak $|\Delta \sigma|$ varies far more weakly (\cref{Fig:p_w_plot}). This subsection explains why.
}

\DIFadd{From \cref{Eq:DeltaSigmaSimple}, the peak $|\Delta\sigma|$ (evaluated at $T_\text{peak}$ where $\alpha_1 = 1$ by \cref{Eq:App:TpeakIsTstar}) can be written as:
}\begin{equation}
\DIFadd{|\Delta\sigma|_\text{peak} = \frac{8}{3} \frac{c_\text{O}}{a^3} \underbrace{\frac{(1-p)}{p} R_v}_{\eta'} \times \underbrace{\frac{[\text{N}_\text{N}^{(\text{s})}]}{[\text{N}_\text{N}^{(\text{b})}]}}_{\text{from } g(T)} \times \exp\!\left(\frac{E_\text{seg}}{kT_\text{peak}}\right) \times |E_\text{ad}(T_\text{peak})|,
\label{Eq:App:DsigPeak}
}\end{equation}
\DIFadd{where $[\text{N}_\text{N}^{(\text{s})}] = \frac{3}{2}(p/(1-p))(a/R_v)$ from \cref{Eq:NNs1}. The product of the two underbraced factors simplifies as:
}\begin{equation}
\DIFadd{\frac{(1-p)}{p} R_v \times \frac{\frac{3}{2}\frac{p}{1-p}\frac{a}{R_v}}{[\text{N}_\text{N}^{(\text{b})}]} = \frac{3a/2}{[\text{N}_\text{N}^{(\text{b})}]},
\label{Eq:App:cancel}
}\end{equation}
\DIFadd{where the factors $(1-p)/p$ and $p/(1-p)$ cancel exactly, and $R_v$ cancels as well. The only remnant is $1/[\text{N}_\text{N}^{(\text{b})}] = 1/(1 - [\text{N}_\text{N}^{(\text{s})}])$, which is close to unity because $[\text{N}_\text{N}^{(\text{s})}] \ll 1$ for the void sizes of interest. Substituting back:
}\begin{equation}
\DIFadd{|\Delta\sigma|_\text{peak} = \frac{4 c_\text{O}}{a^2} \times \frac{1}{[\text{N}_\text{N}^{(\text{b})}]} \times \exp\!\left(\frac{E_\text{seg}}{kT_\text{peak}}\right) \times |E_\text{ad}(T_\text{peak})|.
\label{Eq:App:DsigPeakReduced}
}\end{equation}
\DIFadd{The prefactor $4c_\text{O}/a^2$ is independent of both $p$ and $R_v$. Thus, the peak $|\Delta\sigma|$ depends on porosity only through two weak channels:
}\begin{enumerate}[label=(\alph*)]
\item \DIFadd{The thermodynamic residual $1/[\text{N}_\text{N}^{(\text{b})}]$ deviates from unity only when $[\text{N}_\text{N}^{(\text{s})}] = \frac{3}{2}(p/(1-p))(a/R_v)$ is non-negligible. For $R_v = 25$~nm, this residual varies by less than 1\% across $p = 5$--20\%; for $R_v = 1$~nm, it varies by 18\%.
}\item \DIFadd{Increasing $p$ reduces $\lambda_v$ (voids are closer together), so the kinetic saturation condition $\sqrt{D_1 t} = \lambda_v$ is met at a lower temperature. From \cref{Eq:App:TstarExplicit}:
}\begin{equation}
\DIFadd{T_\text{peak} \sim \frac{1}{\ln\!\left(\ldots\, p^{2/3} \,\ldots\right)},
}\end{equation}
\DIFadd{so $T_\text{peak}$ responds only logarithmically to changes in $p$. Over $p = 5$--20\%, the downward shift is only 31~K for $R_v = 1$~nm and 44~K for $R_v = 25$~nm. This lower $T_\text{peak}$ increases $\exp(E_\text{seg}/kT_\text{peak})$ (stronger segregation drive at lower temperature), which slightly raises $|\Delta\sigma|_\text{peak}$.
}\end{enumerate}

\begin{table}[h!]
\centering
\footnotesize
\caption{\DIFaddFL{Porosity dependence of the peak $|\Delta \sigma|$ at two void radii ($w_\text{O} = 1500$~ppm, intermediate stoichiometry). Each row is evaluated at that porosity's own $T_\text{peak}$ from \cref{Eq:App:TstarFormula}.}}
\label{Tab:App:por}
\begin{tabular}{lcccc}
\hline
& \DIFaddFL{$p$ }[\DIFaddFL{\%}] & \DIFaddFL{$T_\text{peak}$ }[\DIFaddFL{K}] & \DIFaddFL{$1/[\text{N}_\text{N}^{(\text{b})}]$ }& \DIFaddFL{$|\Delta\sigma|$ variation }\\
\hline
\multirow{4}{*}{$R_v = 1$ nm}
& \DIFaddFL{5  }& \DIFaddFL{1448 }& \DIFaddFL{1.040 }& \DIFaddFL{--- }\\
& \DIFaddFL{10 }& \DIFaddFL{1432 }& \DIFaddFL{1.088 }& \DIFaddFL{$+8\%$ }\\
& \DIFaddFL{15 }& \DIFaddFL{1423 }& \DIFaddFL{1.148 }& \DIFaddFL{$+17\%$ }\\
& \DIFaddFL{20 }& \DIFaddFL{1416 }& \DIFaddFL{1.223 }& \DIFaddFL{$+26\%$ }\\
\hline
\multirow{4}{*}{$R_v = 25$ nm}
& \DIFaddFL{5  }& \DIFaddFL{1713 }& \DIFaddFL{1.002 }& \DIFaddFL{--- }\\
& \DIFaddFL{10 }& \DIFaddFL{1690 }& \DIFaddFL{1.003 }& \DIFaddFL{$+4\%$ }\\
& \DIFaddFL{15 }& \DIFaddFL{1678 }& \DIFaddFL{1.005 }& \DIFaddFL{$+7\%$ }\\
& \DIFaddFL{20 }& \DIFaddFL{1669 }& \DIFaddFL{1.007 }& \DIFaddFL{$+8\%$ }\\
\hline
\end{tabular}
\end{table}

\DIFadd{\Cref{Tab:App:por} quantifies both effects for $w_\text{O} = 1500$~ppm at intermediate stoichiometry. The variation column reports the total change in $|\Delta\sigma|_\text{peak}$ (from \cref{Eq:App:DsigPeakReduced}) relative to $p = 5\%$. For $R_v = 25$~nm (the conditions of \cref{Fig:p_w_plot}), the total change in peak $|\Delta\sigma|$ from $p = 5\%$ to $p = 20\%$ is only 8\%, almost entirely from channel~(b). For $R_v = 1$~nm, the total change is 26\%, with 18\% from channel~(a) and 7\% from channel~(b). In both cases, the peak $|\Delta\sigma|$ changes by at most a factor of 1.3 over a fourfold increase in porosity, far smaller than the factor of 4.75 that the na\"ive $(1-p)/p$ scaling would suggest.
}

\FloatBarrier

\DIFaddend \section{Supplementary information}
\label{App3}
...
\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.45\textwidth]{uO.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.45\textwidth]{DO.png}
    \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Oxygen chemical potential as calculated based on \cref{Eq:OxygenTruePot}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(Color online) Diffusivity of oxygen impurities in bulk UN by the defects $ \{\text{O}_\text{N} \! : \! v_\text{N}\} $ and $ \text{O}_i $ under U-rich (blue), Intermediate (orange), and N-rich (green) conditions}\DIFaddendFL .}
    \DIFdelbeginFL %DIFDELCMD < \label{Fig:uO}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{Fig:DO}

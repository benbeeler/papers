...
To study surface properties, 6-layer and 8-layer symmetric slabs based on the same $3 \times 3$ surface unit cell are used, containing 216 and 288 atoms, respectively, where the vacuum gap in each is twice the length of the slab along the $z$-direction. It was shown by Kocevski \textit{et al.}
...
\Cref{Fig:p_w_plot} represents the maximum $|\Delta \sigma|$, which occurs at intermediate stoichiometric conditions. As explained earlier, because $R_v$ is kept fixed, the temperature at which this maximum occurs is approximately the same for all $(w_\text{O}, p)$ combinations shown, varying only between $\sim$1620 and $\sim$1750~K for $R_v$ = 25~nm at intermediate stoichiometry. This weak dependence on $w_\text{O}$ and $p$ follows from \cref{Eq:TpeakMain}, where both parameters enter only through a logarithm. The color map reveals that $|\Delta \sigma|$ is weakly dependent on porosity for $R_v$ = 25 nm. The vertical red line marks the oxygen concentration in UN samples from Ronchi \textit{et al.} ($w_\text{O} = 1800$ ppm) \cite{Ronchi1975,Ronchi1978}, used by Rizk \textit{et al.} \cite{Rizk2025} to validate their swelling model. Fission gas bubbles with an average radius of 25~nm are predicted to experience a reduction in surface energy of about 0.3--0.4~J/m$^2$, approximately 20\% of our calculated surface energy in the oxygen-free case.
...
\begin{itemize}
    ...
    \item \textit{Porosity ($p$):} The surface energy reduction is found to be weakly dependent on porosity.
    \end{itemize}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textit{Existence of $T^*$.} The argument of the logarithm in \cref{Eq:App:TstarFormula} must exceed unity, which is verified for all parameter combinations considered in this work.
\end{enumerate}
...
\begin{equation}
D_1(T) = f\,\frac{[v_\text{N}^{(\text{b})}]}{c_\text{O}}\,D_{\{\text{O}_\text{N}:v_\text{N}\}} = \frac{f\,z\,\lambda^2\,\nu}{6\,c_\text{O}}\,\exp\!\left[-\frac{E_m + E_f(v_\text{N}^{(\text{b})})}{kT}\right],
\label{Eq:App:D1}
\end{equation}
where the second equality uses $D_d = \frac{1}{6}z\lambda^2\nu\exp[-E_m/(kT)]$ and $[v_\text{N}^{(\text{b})}] = \exp[-E_f(v_\text{N}^{(\text{b})})/(kT)]$. In the unsaturated regime ($T < T^*$):
\begin{equation}
\alpha_1(T) = C_\alpha\,\exp\!\left[-\frac{E_m + E_f(v_\text{N}^{(\text{b})})}{2kT}\right], \quad C_\alpha \equiv \frac{1}{\lambda_v}\sqrt{\frac{f\,z\,\lambda^2\,\nu\,t}{6\,c_\text{O}}}.
...
E_\text{seg} \equiv & -\bigl[E_f(v_\text{N}^{(\text{s})}) - E_f(v_\text{N}^{(\text{b})}) + E_\text{ad}(\text{O}_\text{N}^{(\text{s})}) - E_\text{inc}(\text{O}_\text{N}^{(\text{b})})\bigr],
\label{Eq:App:gT}
\end{align}
...
\paragraph{Adsorption-energy factor} From the definition of the adsorption energy (\cref{Eq:Ead}), the adsorption energy referred to the impurity chemical potential is:
\begin{equation}
E_\text{ad}(T) = \frac{1}{N_\text{ad}}\bigl(E_\text{surf+ad} - E_\text{surf}\bigr) - \mu_\text{O}(T),
\end{equation}
where $\frac{1}{N_\text{ad}}(E_\text{surf+ad} - E_\text{surf})$ is the raw (reference-free) adsorption energy, a $T$-independent quantity obtained directly from DFT slab calculations. From \cref{Eq:OxygenTruePot}, using $[\text{O}_\text{N}^{(\text{b})}] \approx c_\text{O}$ and $[v_\text{N}^{(\text{b})}] = \exp[-E_f(v_\text{N}^{(\text{b})})/(kT)]$:
\begin{equation}
...
Substituting:
\begin{equation}
E_\text{ad}(T) = \underbrace{\frac{1}{N_\text{ad}}\bigl(E_\text{surf+ad} - E_\text{surf}\bigr) - E(\text{O}_\text{N}) + E(v_\text{N}) - E_f(v_\text{N}^{(\text{b})})}_{\displaystyle\equiv\,E_\text{ad}^{(0)},\ T\text{-independent}} - kT \ln c_\text{O}.
\label{Eq:App:EadImpExplicit}
\end{equation}
Note that $E_\text{ad}^{(0)}$ is built entirely from raw DFT total energies and the vacancy formation energy; no oxygen chemical potential appears. Since $E_\text{ad} < 0$ (favorable adsorption) and $\ln c_\text{O} < 0$ (dilute impurity), the magnitude $|E_\text{ad}(T)| = -E_\text{ad}^{(0)} + kT\ln c_\text{O}$ is strictly \emph{decreasing} in $T$ (also see \cref{Fig:EduO}), with $d|E_\text{ad}|/dT = k\ln c_\text{O} < 0$.
...
\subsection{Verification of the monotonicity condition}
\label{App:Tpeak:Verification}

The inequality \cref{Eq:App:Inequality} is evaluated for each stoichiometric condition using the DFT values in \cref{Tab:EfVN} and the migration energy $E_m = 3.06$~eV:
\begin{table}
\footnotesize
\centering
\begin{tabular}{lccc}
\hline
 & U-rich & Intermediate & N-rich \\
\hline
$E_m + E_f(v_\text{N}^{(\text{b})})$ [eV] & 4.70 & 5.19 & 5.68 \\
$2E_\text{seg}$ [eV] & 0.66 & 0.66 & 0.66 \\
$\tfrac{1}{2}[E_m + E_f(v_\text{N}^{(\text{b})})] - E_\text{seg}$ [eV] & 2.02 & 2.27 & 2.51 \\
\hline
\end{tabular}
\end{table}
...
\subsection{Closed-form expression for \texorpdfstring{$T^*$}{T*}}
\label{App:Tpeak:Formula}

Solving $\sqrt{D_1(T^*)\,t} = \lambda_v$ using \cref{Eq:App:D1}:
\begin{equation}
T_\text{peak} = T^* = \frac{E_m + E_f(v_\text{N}^{(\text{b})})}{k\,\ln\!\left[\dfrac{f\,z\,\lambda^2\,\nu\,t}{6\,c_\text{O}\,\lambda_v^2}\right]}.
\label{Eq:App:TstarFormula}
\end{equation}
Substituting $\lambda_v = (4\pi R_v^3 / 3p)^{1/3}$:
\begin{equation}
T_\text{peak} = \frac{E_m + E_f(v_\text{N}^{(\text{b})})}{k\,\ln\!\left[\dfrac{f\,z\,\lambda^2\,\nu\,t}{6\,c_\text{O}}\left(\dfrac{3p}{4\pi R_v^3}\right)^{2/3}\right]}.
\label{Eq:App:TstarExplicit}
\end{equation}
The formula is meaningful only when the argument of the logarithm exceeds unity, i.e., when $f\,z\,\lambda^2\,\nu\,t > 6\,c_\text{O}\,\lambda_v^2$. Physically, this means oxygen must be capable of diffusing at least one void spacing within the nucleation time; otherwise $\alpha_1 < 1$ throughout the accessible temperature range and no kinetic saturation occurs. For the parameter ranges in this work ($R_v \in \{1, 10, 25\}$~nm, $p \in [0.05, 0.20]$, $w_\text{O} \in [500, 3000]$~ppm), this condition is satisfied at all relevant temperatures.

\subsection{Dependence of \texorpdfstring{$T_\text{peak}$}{Tpeak} on the nucleation time \texorpdfstring{$t$}{t}}
\label{App:Tpeak:tdep}

\cref{Eq:App:TstarExplicit} makes explicit that $t$ enters the expression for $T_\text{peak}$ only through a logarithm. Differentiating with respect to $\ln t$:
\begin{equation}
\frac{dT_\text{peak}}{d\ln t}
=
...
Taking $E_m + E_f(v_\text{N}^{(\text{b})}) = 5.19$~eV (intermediate stoichiometry) and $T_\text{peak} = 1448$~K (the expected value for $R_v$ = 1 nm, $w_\text{O}$ = 1500 ppm, $p$ = 0.05) gives:
\begin{equation}
\Delta T_\text{peak} \approx -80~\text{K}.
\end{equation}
This confirms that $T_\text{peak}$ is only logarithmically sensitive to the nucleation time.

\subsection{Numerical validation and parametric trends}
\label{App:Tpeak:Validation}

\Cref{Eq:App:TstarFormula} was compared against direct numerical maximization of the full $|\Delta \sigma|(T)$ from \cref{Eq:DeltaSigma1} (including the $\text{O}_i^{(\text{s})}$ and $v_\text{N}^{(\text{s})}$ contributions) for a grid of parameter combinations at intermediate stoichiometry:

\begin{table}[h!]
\centering
\footnotesize
\caption{Comparison of $T_\text{peak}$ from the closed-form expression (\cref{Eq:App:TstarFormula}) and from direct numerical maximization of the full $|\Delta \sigma|(T)$ (\cref{Eq:DeltaSigma1}), at intermediate stoichiometry.}
\label{Tab:App:Tpeak}
\begin{tabular}{lccc}
\hline
 & $T_\text{peak}$ (formula) [K] & $T_\text{peak}$ (numerical) [K] & Difference [K] \\
\hline
$R_v = 1$ nm,  $w_\text{O} = 1500$ ppm, $p = 0.05$ & 1447 & 1448 & $<1$ \\
$R_v = 1$ nm,  $w_\text{O} = 1500$ ppm, $p = 0.20$ & 1416 & 1416 & $<1$ \\
$R_v = 10$ nm, $w_\text{O} = 1500$ ppm, $p = 0.05$ & 1627 & 1627 & $<1$ \\
$R_v = 25$ nm, $w_\text{O} = 1800$ ppm, $p = 0.05$ & 1721 & 1721 & $<1$ \\
$R_v = 25$ nm, $w_\text{O} = 1800$ ppm, $p = 0.20$ & 1676 & 1677 & $<1$ \\
$R_v = 25$ nm, $w_\text{O} = 1500$ ppm, $p = 0.05$ & 1712 & 1712 & $<1$ \\
\hline
\end{tabular}
\end{table}

Agreement is within 1~K in all cases, confirming that the neglected $\text{O}_i^{(\text{s})}$ and $v_\text{N}^{(\text{s})}$ contributions shift the peak negligibly for the DFT parameter set used here.

\Cref{Eq:App:TstarExplicit} yields the following parametric trends, consistent with \cref{Fig:Delta_sigma}:
\begin{itemize}
    \item \textit{Increasing $R_v$} increases $\lambda_v$, decreasing the logarithm and therefore increasing $T_\text{peak}$. The $\sim$180~K upward shift from $R_v = 1$~nm to $R_v = 10$~nm reported in \cref{Sec:Oxygen} is reproduced quantitatively.
    \item \textit{Increasing $p$} decreases $\lambda_v$ and slightly decreases $T_\text{peak}$. The effect is weak because $p$ enters through a logarithm.
    \item \textit{Increasing $c_\mathrm{O}$} increases the denominator of the logarithm's argument and slightly increases $T_\text{peak}$; again, the effect is logarithmic and weak.
    \item \textit{Stoichiometry} enters the numerator through $E_f(v_\text{N}^{(\text{b})})$: N-rich conditions (higher $E_f$) shift $T_\text{peak}$ upward.
\end{itemize}

\subsection{Consequence for the porosity dependence of \texorpdfstring{$|\Delta \sigma|$}{|Delta sigma|}}
\label{App:Tpeak:Porosity}

The prefactor of \cref{Eq:DeltaSigma1} contains the factor $(1-p)/p$, which varies by nearly $4.75\times$ between $p = 5\%$ and $p = 20\%$. Yet the peak $|\Delta \sigma|$ varies far more weakly (\cref{Fig:p_w_plot}). This subsection explains why.

From \cref{Eq:DeltaSigmaSimple}, the peak $|\Delta\sigma|$ (evaluated at $T_\text{peak}$ where $\alpha_1 = 1$ by \cref{Eq:App:TpeakIsTstar}) can be written as:
\begin{equation}
|\Delta\sigma|_\text{peak} = \frac{8}{3} \frac{c_\text{O}}{a^3} \underbrace{\frac{(1-p)}{p} R_v}_{\eta'} \times \underbrace{\frac{[\text{N}_\text{N}^{(\text{s})}]}{[\text{N}_\text{N}^{(\text{b})}]}}_{\text{from } g(T)} \times \exp\!\left(\frac{E_\text{seg}}{kT_\text{peak}}\right) \times |E_\text{ad}(T_\text{peak})|,
\label{Eq:App:DsigPeak}
\end{equation}
where $[\text{N}_\text{N}^{(\text{s})}] = \frac{3}{2}(p/(1-p))(a/R_v)$ from \cref{Eq:NNs1}. The product of the two underbraced factors simplifies as:
\begin{equation}
\frac{(1-p)}{p} R_v \times \frac{\frac{3}{2}\frac{p}{1-p}\frac{a}{R_v}}{[\text{N}_\text{N}^{(\text{b})}]} = \frac{3a/2}{[\text{N}_\text{N}^{(\text{b})}]},
\label{Eq:App:cancel}
\end{equation}
where the factors $(1-p)/p$ and $p/(1-p)$ cancel exactly, and $R_v$ cancels as well. The only remnant is $1/[\text{N}_\text{N}^{(\text{b})}] = 1/(1 - [\text{N}_\text{N}^{(\text{s})}])$, which is close to unity because $[\text{N}_\text{N}^{(\text{s})}] \ll 1$ for the void sizes of interest. Substituting back:
\begin{equation}
|\Delta\sigma|_\text{peak} = \frac{4 c_\text{O}}{a^2} \times \frac{1}{[\text{N}_\text{N}^{(\text{b})}]} \times \exp\!\left(\frac{E_\text{seg}}{kT_\text{peak}}\right) \times |E_\text{ad}(T_\text{peak})|.
\label{Eq:App:DsigPeakReduced}
\end{equation}
The prefactor $4c_\text{O}/a^2$ is independent of both $p$ and $R_v$. Thus, the peak $|\Delta\sigma|$ depends on porosity only through two weak channels:
\begin{enumerate}[label=(\alph*)]
\item The thermodynamic residual $1/[\text{N}_\text{N}^{(\text{b})}]$ deviates from unity only when $[\text{N}_\text{N}^{(\text{s})}] = \frac{3}{2}(p/(1-p))(a/R_v)$ is non-negligible. For $R_v = 25$~nm, this residual varies by less than 1\% across $p = 5$--20\%; for $R_v = 1$~nm, it varies by 18\%.
\item Increasing $p$ reduces $\lambda_v$ (voids are closer together), so the kinetic saturation condition $\sqrt{D_1 t} = \lambda_v$ is met at a lower temperature. From \cref{Eq:App:TstarExplicit}:
\begin{equation}
T_\text{peak} \sim \frac{1}{\ln\!\left(\ldots\, p^{2/3} \,\ldots\right)},
\end{equation}
so $T_\text{peak}$ responds only logarithmically to changes in $p$. Over $p = 5$--20\%, the downward shift is only 31~K for $R_v = 1$~nm and 44~K for $R_v = 25$~nm. This lower $T_\text{peak}$ increases $\exp(E_\text{seg}/kT_\text{peak})$ (stronger segregation drive at lower temperature), which slightly raises $|\Delta\sigma|_\text{peak}$.
\end{enumerate}

\begin{table}[h!]
\centering
\footnotesize
\caption{Porosity dependence of the peak $|\Delta \sigma|$ at two void radii ($w_\text{O} = 1500$~ppm, intermediate stoichiometry). Each row is evaluated at that porosity's own $T_\text{peak}$ from \cref{Eq:App:TstarFormula}.}
\label{Tab:App:por}
\begin{tabular}{lcccc}
\hline
& $p$ [\%] & $T_\text{peak}$ [K] & $1/[\text{N}_\text{N}^{(\text{b})}]$ & $|\Delta\sigma|$ variation \\
\hline
\multirow{4}{*}{$R_v = 1$ nm}
& 5  & 1448 & 1.040 & --- \\
& 10 & 1432 & 1.088 & $+8\%$ \\
...
\Cref{Tab:App:por} quantifies both effects for $w_\text{O} = 1500$~ppm at intermediate stoichiometry. The variation column reports the total change in $|\Delta\sigma|_\text{peak}$ (from \cref{Eq:App:DsigPeakReduced}) relative to $p = 5\%$. For $R_v = 25$~nm (the conditions of \cref{Fig:p_w_plot}), the total change in peak $|\Delta\sigma|$ from $p = 5\%$ to $p = 20\%$ is only 8\%, almost entirely from channel~(b). For $R_v = 1$~nm, the total change is 26\%, with 18\% from channel~(a) and 7\% from channel~(b). In both cases, the peak $|\Delta\sigma|$ changes by at most a factor of 1.3 over a fourfold increase in porosity, far smaller than the factor of 4.75 that the na\"ive $(1-p)/p$ scaling would suggest.

...
\begin{abstract}
Fuel-cladding chemical interaction (FCCI) is a major degradation mechanism in sodium-cooled fast reactors, driven by the diffusion of lanthanide fission products and actinide fuel constituents into Fe-based cladding.  To elucidate this phenomenon, we used density functional theory and self-consistent mean field theory to calculate the vacancy-mediated diffusivities of Ce, Nd, U, and Pu in body-centered cubic (BCC) Fe. Our findings reveal that all solutes exhibit accelerated diffusion relative to Fe, primarily due to strong vacancy drag effects. Additionally, we investigated surface diffusion via the vacancy mechanism on the Fe (110) surface as a simplified model system to study fast diffusion pathways. As expected, the surface diffusion is significantly faster than bulk diffusion, with activation energies reduced by 0.7–0.9 eV. Comparison with literature data on grain-boundary diffusion suggests that the dominant self-diffusion mechanism along GBs remains uncertain, but for oversized solutes, vacancy-mediated diffusion is expected to dominate. These insights provide critical inputs for the development of multiscale FCCI models, advancing our understanding of material degradation in next-generation nuclear reactors. 
\end{abstract}
...
The results of this study provide first-principles-based diffusion data for Ce, Nd, U, and Pu in Fe, supporting the development of multiscale models for the description of FCCI.
...
\begin{equation}
\label{eq:pdc}
D_{pd} = \frac{(1-C_B)}{C_B} \frac{L_{VB}}{L_{AV}} = \frac{(1-C_B)}{C_B} \frac{L_{VB}}{(-L_{VV}-L_{VB})}.
\end{equation}
...
Finally, the attempt frequency from a configuration $i$ to $f$, $\nu^{i\rightarrow f}$, is calculated using a commonly adopted approximation to Vineyard's harmonic transition state theory \cite{vineyard_frequency_1957}, in which only the vibrational modes of the hopping atom(s) are considered rather than the full set of $3N$ degrees of freedom:
...
\end{equation}
where $Q_0^F$ is the activation energy and $D_0$ is the prefactor. This expression is valid at low temperatures well below the Curie temperature of $T_C$ = 1043 K, at which the transition to the paramagnetic state occurs. Diffusion experiments \cite{lubbehusen1990self, hettich1977self} show that at high temperatures closer to $T_C$, magnetic disordering leads to a significant change in the activation energies. Previous theoretical studies also addressed the evolution of diffusion parameters beyond $T_C$ \cite{PhysRevLett.124.215901, PhysRevB.107.094103}. However, in this work, the diffusivities are only calculated up to $T_C$.
Furthermore, it was shown that a non-negligible contribution of electronic excitations plays a role in the diffusion activation energy in BCC metals \cite{satta1998vacancy, sandberg2015modeling}. To include these finite-temperature effects, the corrected activation energy, $Q(T)$, can be written as:
\begin{equation}
...
\end{equation}
where $\tau$ is $T/T_C$, and $P$ is a geometry factor that is 0.4 for BCC \cite{jonsson1992ferromagnetic}. The parameter $\alpha$ is the difference between the activation energies of the ferromagnetic and paramagnetic states ($\alpha = Q^F_0-Q^P$). Using the calculated values for $Q^F_0$ and $Q^P$ by Sandberg \textit{et al.}\cite{sandberg2015modeling} yields $\alpha = 0.87$ eV.
...
The same corrections described in \Cref{eq:corrected_Q} are applied to solute diffusivities via the vacancy mechanism in the dilute limit. This assumes that a single solute atom does not significantly alter the magnetic state and vacancy migration properties, although we acknowledge that this is a strong approximation. At present, there are no available data to build a more precise model of this effect.
The solute diffusivities, both with and without these corrections, are shown in \Cref{fig:diff_Q}. Ce, Nd, U, and Pu all diffuse faster than the Fe self-diffusion, as shown in \Cref{fig:self_diff}. Among these elements, Nd exhibits the highest diffusivity with an activation energy of 2.20 eV, while U diffuses the slowest, with an activation energy of 2.57 eV. 
...
In our calculations, the vacancy formation energy in the third layer of the slab, used as the bulk-like reference, is 2.43 eV, slightly higher than the fully periodic bulk value of 2.22 eV due to residual surface relaxation and finite-slab effects.
...
In contrast to bulk diffusion, surface diffusion includes both Fe–vacancy and solute–vacancy exchange jumps (except for U), and the corresponding attempt frequencies are explicitly evaluated.
For the Fe-vacancy jump, a calculated attempt frequency of 4.8~THz is used (calculated using \Cref{eq:attempt_freq}). For vacancy-solute exchange (\( \omega_{VS} \)), the attempt frequencies are calculated to be 2.7~THz for Ce, 1.3~THz for Nd, and 0.9~THz for Pu. For all other vacancy jumps involving the solutes, the Fe-vacancy attempt frequency of 4.8~THz is applied.
...
While the dominant diffusion mechanism—vacancy, interstitial, or mixed—remains uncertain for GB self-diffusion, we argue that for oversized solutes such as lanthanides and actinides, vacancy-mediated diffusion is expected to be the most favorable pathway due to the high energetic cost of forming interstitial configurations. 
Therefore, the computed surface diffusivities provide a simplified model system that captures key mechanistic trends for lanthanide and actinide mobility along fast diffusion pathways, but they should not be interpreted as quantitative GB diffusivities. Further studies on solute stability and migration along specific GB structures are needed to refine FCCI models at higher length scales.

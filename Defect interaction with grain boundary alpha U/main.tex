...
%While mechanical properties of polycrystalline $\alpha$-U have an inverse relationship with temperature \cite{1974_Sherby}, the formation energy of symmetric tilt GBs of $\alpha$-U shows a positive relationship with temperature and the work of adhesion shows a negative relation \cite{MAHBUBA2021153072}. The presence of grain boundaries can also be correlated to the superconductivity in polycrystalline $\alpha$-U \cite{ma15124106}.
...
\begin{comment}
For instance, segregation of solute atoms into grain boundaries of Cu leads to intergranular embrittlement \cite{balluffi1996}. 

Also, these results help to quantify Xe transport along the planar (GB) or line (dislocation) defects, which leads to Xe bubble formation at the GBs.

\par Grain boundaries act not only as a sink but also as a source for point defects. Diffusional creep within the grain boundary is a result of the absorption (acting as a sink) and emission (acting as source) of vacancies at grain boundaries. This flux is a consequence of differences in vacancy concentrations caused by stresses and can be estimated by the self-diffusivity of the grain boundary \cite{MCFADDEN202066_creep}. 
\end{comment}
...
\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 0.7\textwidth]{Picture1.png}}\\
...
%the msd versus time plot for GBs could not be fitted by a linear function. This is due to the onset of phase transformation induced by the presence of grain boundaries predicted by the utilized interatomic potential. From a coordination analysis, above 750 K atoms within the grains of the bi-crystal attain a crystal structure more similar to the BCC phase, rather than an orthorhombic structure. The radial distribution function changes compared to pure $\alpha$-U, in that the number of peaks is reduced. 

%The effect of the presence of additional point defects on grain boundary diffusion is also explored. In this case, either a single interstitial or a vacancy is inserted within the GB interaction zone, then the diffusivity is calculated following the method stated earlier. 

%As stated before, the GB spatial bin thickness is related to GB interaction length with point defects. The general rule to identify the spatial bin thickness is, at least one  $E_{\mathrm{s}}$ distance on the each side of the GB core as if inserted defect is very far from IL, it will never see the GB. 
...
\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 2.0in]{A_500_Inter.png}}
\subfloat[]{\includegraphics[width = 2.0in]{B_500_Inter.png}}
\subfloat[]{\includegraphics[width = 2.0in]{C_500_Inter.png}}
...
\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 2.0in]{A_500_Vacan.png}} 
\subfloat[]{\includegraphics[width = 2.0in]{B_500_Vacan.png}}
\subfloat[]{\includegraphics[width = 2.0in]{C_500_Vacan.png}}
...
\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 2.25in]{A_SE.png}}
\subfloat[]{\includegraphics[width = 2.25in]{B_SE.png}} 
\subfloat[]{\includegraphics[width = 2.25in]{C_SE.png}}\\
...
% No discernible trend is observed between the temperature and the $E_{\mathrm{s}}$ for either interstitials or for vacancies. 
...
%Comparing low and high-energy GBs of the same type, low energy one always posseses lower segregation energy than the high-energy one, even lower than the $E_{\mathrm{s}}$ of vacancy in high energy GBs. 
...
\begin{table}
\centering
\caption{ Maximum percentage change in segregation energy with respect to 300 K temperature for selected six STGBs of $\alpha$-U. \label{tab:SE}}\\
...
%{\color{red} I think the weighted average figure is not necessary. we are depicting things which don't have a dependence. thus, I think what we should do is rewrite this paragraph and report an average segregation energy for vacancies and interstitials that is weighted on $E_{\mathrm{gb}}$. i have rewritten some of this}
...
\begin{figure}[h!]
\centering
%\subfloat[]{\includegraphics[width = 2.5in]{simple_SI.png}}
\subfloat[]{\includegraphics[width = 2.5in]{weighted_SI.png}} 
%\subfloat[]{\includegraphics[width = 2.5in]{simple_SV.png}} 
\subfloat[]{\includegraphics[width = 2.5in]{weighted_SV.png}}
...
\begin{comment}
In this method, the probability of the presence of a GB depends on its formation energy \cite{WILLIAMS201545}. The weighted value of the formation energies of the GBs from the same type (for example, in the case of type A GBs, only the formation energy of GB A1 and A2) are used as the weighting parameters while taking the average of their properties such as point segregation energy, interaction distance, etc. $E_{\mathrm{gb}}$ for different type of GBs at different temperature is taken from \Cref{fig:GBE}. A similar approach is applied for the type B and type C GBs. This method is noted as the "weighted average" method for the rest of the article and used the following equation :

\begin{table}[h!]
    \centering
    \caption{Grain boundary type, segregation energy, and weighted average segregation energy. }
    \label{tab:weight}
    \begin{tabular}{|c|c|c|}
        \hline
        GB Type & SE (eV) & Weighted Average SE (eV) \\
        \hline
        A1 & X & \multirow{ 2}{*}{X} \\
        A2 & X & \\ \cline{3-3}
        B1 & X & \multirow{ 2}{*}{X} \\
...
The average segregation energy of the point defect at the GB core of the three different types of GBs is calculated considering a weighted average depending on GB energies. These values are plotted as a function of temperature in \Cref{fig:SE}. Error bars in these figures are twice the propagated standard error. 
%As while averaging only two GBs with highest and lowest formation energy from each type is considered, error bar for the GB type average $E_{\mathrm{s}}$ becomes as high as ~2eV. By adding more GBs in the study, this error bar can be minimized. These larger error bar.

\begin{equation}
\label{eq:weight}
SE_{weighted avg, x ,y} = \frac{FE_{x1}}{FE_{x1}+FE_{x2}} \times SE_{x1,y} + \frac{FE_{x2}}{FE_{x1}+FE_{x2}} \times SE_{x2,y}
\end{equation}
where, $x$ denotes GB type and $y$ denotes point defect type.

Segregation energy of interstitial shows no dependency on GB type when "weighted average" method is incorporated. Simialer phenomena is observed for vacancy. These results denote, the value of average segregation energy of point defect in the GB of alpha U might be considered as GB orientation and temperature independent but $E_{\mathrm{gb}}$ dependent in the larger scale simulations.

\begin{figure}
\centering
\includegraphics[width = 3in]{GBE.png}\\
\caption{Formation energy of A1, A2, B1, B2, C1, and C2 grain boundaries of $\alpha$-U as a function of temperature. Values are taken from Reference \cite{MAHBUBA2021153072}.}
\label{fig:GBE}
\end{figure}
...
\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 2.25in]{A_IL.png}}
\subfloat[]{\includegraphics[width = 2.25in]{B_IL.png}} 
\subfloat[]{\includegraphics[width = 2.25in]{C_IL.png}}\\
...
\multicolumn{1}{|c}{ } 
 ...
 & \multicolumn{1}{c|}{C2} \\
 \hline
Vacancy & -- & 40.4 & 60 & 18.2 & -- & 38.5\\
Interstitial & 62.5 & 0.0 & 29.7 & 18.8 & 47.1 & 13.6\\
\hline
\end{table}
\ On the other hand,  $l_{\mathrm{interaction}}$ of A1 and C1 for interstitial lies between 15 {\AA} to 21 {\AA}.par On the other hand, low formation energy GBs (B1, C1) possess a longer range of interaction with single interstitial as the temperature increases. One exception is low energy A1 grain boundary. A1 GB becomes less prominent sink at higher operating temperature because of decrease in both $E_{\mathrm{s}}$ and IL. Maximum change is  $l_{\mathrm{interaction}}$ for single interstitial is noted for A1 GB with respect to 300 K temperature (see table \ref{tab:IL}). Moreover, in general low energy GBs of $\alpha$-U shows more sensitivity to temperature in $E_{\mathrm{s}}$ of single interstitial,  $l_{\mathrm{interaction}}$ of singe interstitial and formation energy compared to high energy GBs. For high formation energy GBs (A2, B2, and C2), the interaction length with single interstitial reduces as temperature increases (maximum by 18.8\% compared to 300K) though the formation energy of the grain boundaries (see figure \ref{fig:GBE}) increases with temperature. For these three GBs (A2, B2, and C2), $E_{\mathrm{s}}$ of single interstitial also slightly increases (maximum by 32.5\% compared to 300K). 


\begin{figure}
\centering
\includegraphics[width = 3in]{IL_GB.png}\\
\caption{The interaction length of point defects with grain boundaries in $\alpha$-U at different studied temperatures plotted as a function of the corresponding grain boundary energy.}
\label{fig:IL_GB}
\end{figure}

\par As in Fig. \ref{fig:IL_GB}, the  $l_{\mathrm{interaction}}$ at all temperatures is plotted over the respective $E_{\mathrm{gb}}$ in Fig. \ref{fig:IL_GB}. A straight line with positive slope is observed for interstitial and for vacancy slope of the straight line is negative. Value of the increase rate (or decrease rate) for interstitial (or vacancy) is approximately 13.74 {\AA}/J/m${^2}$ (or 22.85 {\AA}/J/m${^2}$). As $E_{\mathrm{gb}}$ increases, there is more misfit within the GB, effect of this misfit strain is longer (as larger IL) and more (as higher SE) for interstitial. On the other hand, though $E_{\mathrm{s}}$ of vacancy increases with $E_{\mathrm{gb}}$,  $l_{\mathrm{interaction}}$ decreases as $E_{\mathrm{gb}}$ increases. Decreasing the  $l_{\mathrm{interaction}}$ and increasing the $E_{\mathrm{s}}$ are two opposite phenomenons for GBs acting as a sink.
\end{comment}
...
%This finding may lead to that, width corresponding to the excess energy due to the presence of GBs, and additional excess energy due to the presence of the point defect within the GB are not same. This discrepancies between doubled $l_{\mathrm{int}}$ and $l_{\mathrm{sb}}$, may lead to the large error bar in figure \ref{fig:IL}. The information on the $E_{\mathrm{s}}$ and IL, including the $E_{\mathrm{gb}}$ dependence of these properties, can be utilized to parameterize cluster dynamics and phase field models describing complex defect evolution through a realistic microstructure in $\alpha$-U.

\begin{figure}[h!]
\centering
%\subfloat[]{\includegraphics[width = 2.5in]{simple_IL_SI.png}}
\subfloat[]{\includegraphics[width = 2.5in]{weighted_IL_SI.png}}
%\subfloat[]{\includegraphics[width = 2.5in]{simple_IL_SV.png}} 
\subfloat[]{\includegraphics[width = 2.5in]{weighted_IL_SV.png}}
\caption{Weighted average $l_{\mathrm{int}}$ of GBs in $\alpha$-U with (a) interstitial and (b) vacancy plotted over the temperature.}
\label{fig:IL}
\end{figure}

%with GBs is plotted as a function of the temperature in \Cref{fig:IL}. There appears to be a difference in the GB type and the associated IL, in disagreement with the results for $E_{\mathrm{s}}$ in \Cref{fig:SE}. 


%This trend is consistent with temperature, but may not be statistically significant. Similar to the SE, the  $l_{\mathrm{interaction}}$ of point defects has no dependence on the temperature. The anisotropy of $E_{\mathrm{gb}}$-weighted interaction lengths, combined with the $E_{\mathrm{gb}}$-weighted SEs from \Cref{fig:SE} can be utilized to parameterize cluster dynamics and phase field models describing complex defect evolution through a realistic microstructure in $\alpha$-U.


%Utilizing the "simple averaging" technique, for vacancy, type B shows the highest IL, followed by type A and type C. Similar relationship held for the "weighted averaging" method too. In the case of interstital, first method shows highest  $l_{\mathrm{interaction}}$ for type B, followed by type C and then type A same as $E_{\mathrm{s}}$ of interstitial in the GB core. For the second method this relationship is altered to highest in type B and C followed by type A at all of the studied temperatures . Similar to the $E_{\mathrm{s}}$ of point defects in GB core,  $l_{\mathrm{interaction}}$ of point defects has no dependence on the temperature, though  $l_{\mathrm{interaction}}$ has more anisotropic behaviour than SE. 

%So based on the results from the current work, in a type B GB rich coarse grain (grain radius 14 {\AA}) poly-crystal $\alpha$-U sample, if the operating temperature is between 300 K to 600 K, then GBs within the sample will act as a better sink than a type A or C rich sample. 
...
%The self-diffusion of $\alpha$-U atoms and diffusion of point defects from Wang \cite{WANG2023154289} are included here for the comparative analysis. 
...
%Utilizing these standard errors, bounds on the migration energy and pre-exponential factor can be determined.

\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 2.25in]{A1_diffusion.png}}
\subfloat[]{\includegraphics[width = 2.25in]{B1_diffusion.png}} 
\subfloat[]{\includegraphics[width = 2.25in]{C1_diffusion.png}}\\
...
%Usually, pipe diffusion is observed along the line direction of an edge dislocation. As studying the structure of the GBs is not within the scope of the current study, more details of the observed pipe diffusion are not discussed here. 
...
\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 2.25in]{A2_diffusion.png}}
\subfloat[]{\includegraphics[width = 2.25in]{B2_diffusion.png}} 
\subfloat[]{\includegraphics[width = 2.25in]{C2_diffusion.png}}\\
...
%1.3 times for the A2, 2.2 times for the B2, and 1.2 times for the C2 GB. 
...
%for the A2 GB by an average of 1.4 times and the B2 GB by an average of 2 times. No significant changes are observed for the C2 GB for the GB+V. 
% while also being the least affected by point defects. 
The C2 GB remains the fastest diffusion path, including the GB+I and GB+V conditions. 
%Unlike the low energy GBs, GB+V condition shows more diffusion than the GB+I condition (1.24 times) at 750 K for high energy GBs.
%The ratio between enhancement in self-diffusivity due to the presence of interstitial and vacancy is 0.9 (average as standard error is only 0.02) for A2 GB, between 1.2 to 0.9 for B2 GB, and between 1.5 to 0.9 for C2 GB. 



%\par While accelerated diffusion is observed for the GB+I condition in low-energy GBs, this is not true for high-energy GBs. For the C2 GB, two inflection temperatures (572 K and 687 K), for B2 GB one inflection temperature (572 K), and no inflection temperatures are found for A2 GB, see figure \ref{fig:Dif_fit} (f), (e), and (d) respectively. When the temperature is below 572 K, GB+V is the slowest diffusion mechanism for C2 GB and GB+V becomes fastest above 687 K. GB+V becomes the fastest diffusion condition above 572 K for the B2 GB, though the ratio is maximum 1.1. For A2 GB, GB+V always shows the fastest diffusivity followed by GB+I and only GB condition.

\FloatBarrier
\subsubsection{Discussion}

%Low-energy GBs A1 and C1 have the lowest diffusivity among the studied GBs, while the C2 GB has the highest diffusivity. 
...
The reason behind utilizing the weighted average technique over simple average is, in the current study two very low energy GBs (A1 and C1) are considered, which also show the lowest diffusivity. As such, simple average diffusivity through low energy ( $<$ 0.7 J/m${^2}$ ) and high energy GBs ( $>$ 0.85 J/m${^2}$) are calculated also and shows in the following \Cref{eq:lowhigh,eq:lowhigh1}. Within the temperature range of 500 K to 750 K, high energy GBs has approximately 3.9 times higher diffusivity than the low energy GB.

\begin{equation}
\label{eq:lowhigh}
D_{GB,low energy} = 2.11 \times 10^{-10} exp(\frac{-0.197}{k_{B} T})
\end{equation}

\begin{equation}
\label{eq:lowhigh1}
D_{GB,high energy} = 8 \times 10^{-10} exp(\frac{-0.197}{k_{B} T})
\end{equation}
%(only GB, GB with vacancy, and GB with interstitial), 

\end{comment}

\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 2.8in]{final2.png}}
%\subfloat[]{\includegraphics[width = 2.2in]{final.png}} 
\subfloat[]{\includegraphics[width = 2.8in]{final1.png}} 
...
%diffusivity of the same type of GBs of $\alpha$-U, and (c) weighted averaged diffusivity of the same type of GBs of $\alpha$-U is plotted over the temperature.}
\label{fig:final}
\end{figure}
...
\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[height = 2.4in]{A_diffusion.png}}
\subfloat[]{\includegraphics[height = 2.4in]{B_diffusion.png}} 
\subfloat[]{\includegraphics[height = 2.4in]{C_diffusion.png}}\\
...
\begin{figure}[h!]
\centering
\includegraphics[width = 4in]{4_Andrea.png}\
\caption{diffusivity vs migration energy at different temperatures.}
\label{fig:Andrea}
\end{figure}

\end{comment}

%Among the studied GBs, A1+SV, B1+SV, and C1+SV have the higher migration energy than vacancy (0.434 eV) and interstitial (0.42eV). $\alpha$-U atoms within the A1,B1, and C1 GBs have higher acceleration rate of self diffusion through the GB in the presence of vacancy as the temperature accelerate compared to point defect diffusion through bulk $\alpha$-U. However, point defect diffusion rate have high order of magnitude because of larger pre-factor. 



%The $E_{\mathrm{m}}$ for GB diffusion is significantly lower than the $E_{\mathrm{m}}$ for self-diffusion in the $\alpha$-U lattice. A classical MD study reported a value of approximately 1.83 eV \cite{WANG2023154289}, a DFT calculation found the $E_{\mathrm{m}}$ to be 2.33 eV \cite{Huang_2011}, and experimental studies on self-diffusion showed that the migration energy varies between 1.91 eV and 2.91 eV \cite{Huang_2011}. 
...
%But the pre-factor (which contains jump distance, frequency, etc. \cite{was2007}) of GB self-diffusion is two to three orders lower than the point defects diffusion in the bulk. On the other hand, 
%Due to the larger $E_{\mathrm{m}}$ of the self atom (1.64 eV \cite{WANG2023154289}, considering the constant formation energy of interstitial and vacancy at 500 K which is the lowest temperature in the current GB diffusion study), self-diffusion of $\alpha$-U atom in bulk system is 10$^{11}$ times slower than the $\alpha$-U atom within the GB. 
%This points towards the decrease in the $E_{\mathrm{m}}$ as the likely component yielding accelerated GB diffusion compared to the bulk (or lattice) diffusion. 

%GBs A1 and B1 have a higher migration energy in the presence of vacancy than the vacancy migration energy in the bulk. This phenomena is observed for symmetric tilt GBs of Mo in the presence of interstitial utilizing MD \cite{Novoselov2014}. The reason behind this is hypothesised that, when the displacement of a defect at the GB is accompanied by a rotation,, it will lead to an increase in ME, otherwise not \cite{Novoselov2014}.

%the self-diffusion of the $\alpha$-U GBs is significantly lower than the migration energy for the self diffusion of the $\alpha$-U lattice (approx. 1.83 eV as per reproduced data from MD calculation by Wang et al. \cite{WANG2023154289}, from DFT calculation this value considering isotropic diffusion is 2.33 eV \cite{Huang_2011}, as per experimental studies self-diffusion migration energy varies between 1.91 eV to 2.91 eV \cite{Huang_2011}). A similar trend is observed for bcc iron \cite{baufil_bccFe} where .... 
...
% Meyer-Neldel energy is the inverse of the slope of the fitted linear line, which is calculated as approximately 88 meV in the current study. 

\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 4in]{compensation.png}}\\
...
%Among the high-energy GBs, diffusion through the A2 GB is the slowest and among the low-energy GBs, B1 shows the highest diffusion rate. Below 633 K temperature, diffusion of $\alpha$-U atoms though B1 GB dominates over A2 GB (maximum by 1.7 times at 500 K).

%\par Above 750 K, the msd versus time plot for GBs could not be fitted by a linear function. This is due to the onset of phase transformation induced by the presence of grain boundaries predicted by the utilized interatomic potential. From a coordination analysis, above 750 K atoms within the grains of the bi-crystal attain a crystal structure more similar to the BCC phase, rather than an orthorhombic structure. The radial distribution function changes compared to pure $\alpha$-U, in that the number of peaks is reduced. Thus, our study is constrained to a maximum temperature of 750 K. 


%Also, B1 GB with SPD, from coordination analysis, at 750 K and 800 K temperature, atoms within the grains of the bi-crystal attain a crystal structure more similar to the BCC phase, rather than an orthorhombic structure. The radial distribution function changes compared to pure $\alpha$-U, for instance, the number of peaks is reduced while increasing the temperature. As a result, the maximum temperature for the B1 GB is 700 K. A2 and C2 with SPD (both interstitial and vacancy) at 800 K shows the similar characteristics. These phenomena indicate that GBs with a higher formation energy show early phase transformation. Low energy GBs (probable twin) A1 GB with single vacancy shows no diffusion below 600 K. As such 600 K is the lowest temperature for self-diffusion of A1 GB in the presence of a single vacancy.



%For Cu and Nb, GB self-diffusivity data from molecular dynamics calculation is available in units of GB width times diffusivity, though the studied temperature is much higher (950 K to 1150 K) \cite{cu_nb} than the studied temperature of the current work. Apart from this, migration energy (ME) is not constant for all temperature, typically at higher temperature diffusivity accelerates more \cite{sub-arrhenious}. For these reasons So, GB self diffusivity of Cu and Nb is not compared directly here.
...
\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 3.0in]{1.png}}
\subfloat[]{\includegraphics[width = 3.0in]{3.png}}\\
...
High formation energy GB always posses higher $E_{\mathrm{s}}$ for SPD at the GB core, so the ratio between $E_{\mathrm{s}}$ of a specific SPD type at high energy and low energy GB core of similar type is always greater than 1. Taking the average over the studied temperature range this ration for single interstitial is calculated as 6.1, 1.24, and 4.7 for type A, type B, and type C GBs. In the case of single vacancy this become 3.5 for type B GBs as two coherent GBs with low formation energy GBs (A1 and C1) show no segregation of single vacancy.

\begin{figure}
\centering
\subfloat[]{\includegraphics[width = 2in]{Com_S.png}}
\subfloat[]{\includegraphics[width = 2in]{Com_I.png}} 
\subfloat[]{\includegraphics[width = 2.22in]{Com_D.png}}\\
\caption{}
\label{fig:Dif}
\end{figure}
...
%\par The grain boundary structure plays an important role point defect enhances self-diffusion of the high formation energy GBs (A2, B2, and C2) whereas it introduces the self-diffusion in the low energy one (A1, B1, and C1).
...
\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics[width = 4.5in]{PRE-EXPONENT.png}}
\caption{Calculated pre-exponent constant of the Arrhenius fit of self diffusion of the six studied GBs of $\alpha$-U, each for three different conditions. Condition GB means only GB, GB+V means GB with presence of vacancy and GB+I means GB with a presence of interstitial. Pre-exponent constants are listed here for average diffusion, maximum diffusion and minium diffusion of the GBs.}
\label{fig:pre}
\end{figure}

\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics [width = 3 in] {activation.png}}
\caption{Calculated migration energy of the Arrhenius fit of self diffusion of the six studied GBs of $\alpha$-U, each for three different conditions. Condition GB means only GB, GB+V means GB with presence of vacancy and GB+I means GB with a presence of interstitial. migration energies are listed here for average diffusion, maximum diffusion and minium diffusion of the GBs. Migration energies listed here are plotted in Fig. \ref{fig:Migration}.}
\label{fig:pre}
\end{figure}
\begin{figure}[h!]
\centering
\subfloat[]{\includegraphics [width = 2.2 in] {cobble_act.png}}
\subfloat[]{\includegraphics [width = 2 in]{cobble_pre.png}}
\caption{Calculated (a) activation energy of Coble creep and (b) pre-factor as per equation \ref{eq:coble2} of the six studied GBs of $\alpha$-U. Only average selve diffusion through the each of the six grain boundaries are considered here.}
\label{fig:creep}
\end{figure}
\end{comment}

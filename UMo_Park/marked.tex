...
%DIF DEL first.tex   Wed Sep 28 12:03:25 2022
%DIF ADD main.tex    Wed Sep 28 13:41:04 2022
...
\usepackage{lineno}\linenumbers %DIF > 
...
%DIFDELCMD < %%%
\begin{displaymath}
\DIFdel{%DIFDELCMD < \label{eq:surface2}%%%
...
\begin{equation}
\label{eq:beta}
\beta_{n} = \frac{4\pi(r_{0}+r_{n})D_{v}}{V_{at}}\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}

\begin{equation}
\label{eq:alpha}
\alpha_{n+1} = \beta_{n}\textrm{exp}(\frac{-E_{n+1}^{b}}{kT})\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
\\
...
\DIFaddend The steady-state concentrations of Xe-vacancy clusters were dependent on the temperature, exhibiting three distinct regimes. In the \DIFdelbegin \DIFdel{low temperature regime}\DIFdelend \DIFaddbegin \DIFadd{low-temperature regime, }\DIFaddend the smallest Xe-vacancy cluster (the Xe-monovacancy cluster) had the highest concentration, followed by the Xe-divacancy cluster, the Xe-trivacancy cluster, and the Xe-quadvacancy cluster. This corresponds to temperatures less or equal to 300 K at 5$\times$10$^{18}$ fiss/m$^{3}$/s, \DIFdelbegin \DIFdel{350 }\DIFdelend \DIFaddbegin \DIFadd{325 }\DIFaddend K at 5$\times$10$^{20}$ fiss/m$^{3}$/s, and 400 K at 5$\times$10$^{22}$ fiss/m$^{3}$/s. In the \DIFdelbegin \DIFdel{low temperature }\DIFdelend \DIFaddbegin \DIFadd{low-temperature }\DIFaddend regime, there are a sufficient number of vacancies present to allow for clustering due to the low recombination rate constant (Fig. \ref{fig:recomb}(b)). However, a small Xe-vacancy cluster is not able to absorb a large number of vacancies and grow into a bigger vacancy cluster since the diffusion of vacancies is extremely slow. In the intermediate temperature range, there are still a sufficient number of vacancies present, and the diffusion of vacancies is sufficiently high to allow for absorption of vacancies by an existing Xe-vacancy cluster, resulting in the highest concentration of the biggest Xe-vacancy cluster\DIFdelbegin \DIFdel{, followed by the Xe-trivacancy cluster, the Xe-divacancy cluster, and the Xe-monovacancy cluster}\DIFdelend . This intermediate regime corresponds to the temperatures between \DIFdelbegin \DIFdel{350 }\DIFdelend \DIFaddbegin \DIFadd{325 }\DIFaddend K and 500 K at 5$\times$10$^{18}$ fiss/m$^{3}$/s, 400 K and \DIFdelbegin \DIFdel{550 }\DIFdelend \DIFaddbegin \DIFadd{575 }\DIFaddend K at 5$\times$10$^{20}$ fiss/m$^{3}$/s, and 450 K and \DIFdelbegin \DIFdel{650 }\DIFdelend \DIFaddbegin \DIFadd{625 }\DIFaddend K at 5$\times$10$^{22}$ fiss/m$^{3}$/s. In the \DIFdelbegin \DIFdel{high temperature }\DIFdelend \DIFaddbegin \DIFadd{high-temperature }\DIFaddend regime, the concentration of all of the existing Xe-vacancy clusters started to decrease as the temperature increased. The population of vacancies was dramatically reduced at high temperatures due to the high recombination rate constant and the high diffusivity leading to Frenkel pair annihilation. Thus, there are not enough residual vacancies to interact with the Xe species. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{As the temperature increased, the }\DIFaddend concentration of larger Xe-vacancy clusters decreased more rapidly than the concentration of smaller Xe-vacancy clusters. As a result, the concentration of the biggest Xe-vacancy cluster \DIFdelbegin \DIFdel{, the Xe-quadvacancy cluster, }\DIFdelend had the lowest concentration, while the concentration of the smallest Xe-vacancy cluster, Xe-monovacancy cluster, had the highest concentration. The \DIFdelbegin \DIFdel{temperature regime where the smallest cluster had the highest concentration and the biggest cluster had the lowest concentration }\DIFdelend \DIFaddbegin \DIFadd{high temperature regime }\DIFaddend corresponds to temperatures greater than \DIFdelbegin \DIFdel{or equal to 600 }\DIFdelend \DIFaddbegin \DIFadd{575 }\DIFaddend K at 5$\times$10$^{18}$ fiss/m$^{3}$/s, \DIFdelbegin \DIFdel{650 }\DIFdelend \DIFaddbegin \DIFadd{625 }\DIFaddend K at 5$\times$10$^{20}$ fiss/m$^{3}$/s, and \DIFdelbegin \DIFdel{750 }\DIFdelend \DIFaddbegin \DIFadd{700 }\DIFaddend K at 5$\times$10$^{22}$ fiss/m$^{3}$/s. Thus, these three \DIFdelbegin \DIFdel{temperatures }\DIFdelend \DIFaddbegin \DIFadd{temperature }\DIFaddend regimes are delineated by changes in the mobility of vacancies \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the temperature dependence of the recombination rate\DIFdelbegin \DIFdel{, as well as the vacancy and Xe production rate}\DIFdelend . It should be emphasized that these trends are applicable for all Xe cluster sizes investigated (up to the \DIFdelbegin \DIFdel{Xe-quadvacancy }\DIFdelend \DIFaddbegin \DIFadd{Xe-tetradecavacancy }\DIFaddend cluster), as displayed in Fig. \DIFdelbegin \DIFdel{\ref{fig:Xecon} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:FEBE} }\DIFaddend for clarity. \\

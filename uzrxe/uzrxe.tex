...
\subsection{Depletion of Zr at void surface}

Given the substantially different surface energy as a function of composition, it could be inferred that U-rich surfaces are preferred. However, from Fig. \ref{fig:Efperat}, the highest energy configuration is near 10 {\%} Zr. Thus, for a system with a Zr content of 15 \%, for example, the bulk systems will want to incorporate additional Zr. As such, there does not appear to be an energetic penalty for depleting the surfaces of Zr, leading to U-rich surfaces. To explore this phenomena, we explored a system of U-10Zr (23 atomic percent), at 1200 K, and extended the simulation time out to 1 ns. 
...
\end{equation} where $k_B$ is the Boltzmann constant, $T$ is the temperature (K), $\gamma=0.8$ is the surface tension of the fuel (J/m$^2$) \cite{karahan2009}, $Y_{gas}=0.3017$ is the gaseous fission product yield, $\dot{F}$ is the fission rate density, $t$ is time, and $N$ is the number density of bubbles. The fuel is assumed to exhibit a constant number density of bubbles that grow as a function of burnup and the bubbles are assumed to be in mechanical equilibrium with the solid \cite{LLS_report} \textcolor{red}{need a citation here}. The porosity of the system is directly related to the gaseous swelling: 
...
\end{center}
\caption{Comparison of porosity as a function of fuel surface tension.}
